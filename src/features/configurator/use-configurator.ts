...
export const BEVERAGE_LABEL: Record<Beverage, string> = {
  lager: "Lager",
  beer: "Ale or stout",
  cider: "Cider",
  soft: "Soft drinks",
...
export function useConfiguratorData() {
  ...
  return {
    rules: useQuery({
      queryKey: queryKeys.configuratorRules(key),
      queryFn: () => api.configurator.rules(),
      staleTime: Infinity,
    }),
    configurations: useQuery({
...
export function initialState(rules: ConfiguratorRules, siteAddressId: string | null): BuilderState {
  const points: DispensePoint[] = Array.from({ length: rules.defaults.points }, (_, i) => ({
    id: `pt-${i + 1}`,
    name: `Dispense point ${i + 1}`,
    taps: [...rules.defaults.beveragesPerPoint],
  }));
  ...
  const state: BuilderState = {
    name: "",
    venueType: rules.defaults.venueType,
    siteAddressId,
    selections,
  };
...
export function fromConfiguration(c: Configuration, copy = false): BuilderState {
  return {
    ...
    selections: structuredClone(c.selections),
  };
}
...
): { state: BuilderState; removed: string[] } {
    ...
    (current: { state: BuilderState | null; removed: string[] }, action: BuilderAction) => {
      ...
      switch (action.type) {
        ...
        case "select": {
          ...
          if (action.on)
            b.optionIds = [
              ...(action.single ? b.optionIds.filter((id) => !inGroup.includes(id)) : b.optionIds),
              action.optionId,
            ];
...
export function useBillOfMaterials(
  ...
  rules: ConfiguratorRules | undefined,
  priceLines: { productId: string; price: Money }[] | undefined,
) {
  return useMemo(() => {
