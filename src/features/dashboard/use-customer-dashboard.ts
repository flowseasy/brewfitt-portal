...
export function frequentProducts(orders: SalesOrder[], priceList: PriceListResponse, limit = 5): FrequentProduct[] {
  ...
  return [...stats.entries()]
    ...
    .sort((a, b) => b[1].orders - a[1].orders || b[1].lastOrderedAt.localeCompare(a[1].lastOrderedAt))
    .slice(0, limit)
...
export function useCustomerDashboard() {
  ...
  const derived = useMemo(() => {
    const byCategory = (category: AIInsight["category"]) => (insights.data ?? []).filter((i) => i.category === category);
    const openInvoices = (invoices.data ?? []).filter((i) => i.outstanding.amount > 0 && i.kind === "invoice").sort((a, b) => a.dueAt.localeCompare(b.dueAt));
    const openOrders = (orders.data ?? []).filter((o) => OPEN_ORDER_STATUSES.includes(o.status));
    return {
      nextPayment: openInvoices[0] ?? null,
      awaitingQuotes: (quotes.data ?? []).filter((q) => q.status === "sent").sort((a, b) => a.validUntil.localeCompare(b.validUntil)),
      openOrders,
      stageCounts: {
      ...
      nextDeliveries: [...openOrders].sort((a, b) => (a.confirmedDate ?? a.requestedDate).localeCompare(b.confirmedDate ?? b.requestedDate)).slice(0, 4),
      ...
      activeJobs: (jobs.data ?? []).filter((j) => j.status === "scheduled" || j.status === "in-progress").sort((a, b) => a.scheduledDate.localeCompare(b.scheduledDate)),
      activity: (notifications.data ?? []).slice(0, 6),
    };
  }, [insights.data, invoices.data, quotes.data, orders.data, priceList.data, threads.data, cases.data, jobs.data, notifications.data]);

  return { me, statement, invoices, quotes, orders, priceList, insights, threads, cases, jobs, notifications, ...derived };
}
