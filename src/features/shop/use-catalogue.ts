...
export const SECTION_LABEL: Record<CatalogueSection, string> = { cellar: "Cellar", bar: "Bar", "mobile-dispense": "Mobile dispense" };
...
const FEATURED_CATEGORY_ORDER = ["Fonts", "Taps", "Keg Couplers", "Regulators", "Line Cleaning Equipment", "Beer Coolers", "Drip Trays", "CoolTube", "Coolants", "Lindr Dispenser", "Nitro Dispense", "Badge Holders", "Clamp Assemblies", "Foam Stop", "Gas Chain", "Bar Lighting", "Hot Drinks", "Wine Cooler"];

export function filterCatalogue(
  lines: CatalogueLine[],
  categories: Category[],
  f: { search: string; section: CatalogueSection | "all"; category: string | "all"; inStockOnly: boolean; sort: SortKey },
): CatalogueLine[] {
  ...
  const scored = lines
    .filter((l) => {
      ...
      if (f.category !== "all" && p.category !== f.category && p.subcategory !== f.category) return false;
      if (f.inStockOnly && l.stock?.status !== "in-stock" && l.stock?.status !== "low") return false;
      if (!words.length) return true;
      const hay = `${p.name} ${p.sku} ${p.description} ${catById.get(p.category)?.name ?? ""} ${catById.get(p.subcategory ?? "")?.name ?? ""}`.toLowerCase();
      return words.every((w) => hay.includes(w));
    })
    .map((l) => ({ l, score: words.length ? (l.product.name.toLowerCase().includes(term) ? 2 : 0) + (l.product.sku.toLowerCase() === term ? 3 : 0) : 0 }));
  const sorted = scored.sort((a, b) => {
    switch (f.sort) {
      ...
      default: {
        if (words.length) return b.score - a.score || a.l.product.name.localeCompare(b.l.product.name);
...
export function useBasketMutations() {
  ...
  const update = useMutation({
    mutationFn: (patch: Parameters<typeof api.shop.updateBasket>[0]) => api.shop.updateBasket(patch),
    onMutate: async (patch) => {
      ...
      if (previous) queryClient.setQueryData<Basket>(queryKeys.basket(key), { ...previous, ...patch, lines: patch.lines ?? previous.lines } as Basket);
      return { previous };
    },
