...
function Tour() {
  ...
  return (
    <main className="relative flex min-h-dvh flex-col overflow-hidden">
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 top-0 h-[380px] bg-gradient-to-b from-brand-subtle to-transparent"
      />
        ...
        >
          <div>
            ...
            <h1
              ...
              ref={headingRef}
              tabIndex={-1}
              className="text-3xl font-semibold tracking-tight text-balance outline-none sm:text-4xl"
            >
            ...
            {steps.map((s, i) => (
              <li key={s.id}>
                <button
                  ...
                  className={cn(
                    "block h-2 rounded-full transition-all focus-visible:ring-3 focus-visible:ring-ring/40 focus-visible:outline-none",
...
function useSteps(supplier: boolean): Step[] {
  ...
  return [
    ...
    },
    {
      ...
      body: supplier
        ? "Open requests for quotation, purchase orders by stage, what Brewfitt owes you and how your range is moving."
        : "Quotes waiting for you, orders on their way, your account balance and anything that needs a reply, the moment you sign in.",
    ...
    supplier
      ? {
          id: "products",
          icon: CubeIcon,
          eyebrow: "Products and RFQs",
          title: "Quote, submit and keep your range current",
          body: "Respond to Brewfitt's requests for quotation with price and lead time, submit new products and offers for approval, and see Brewfitt's stock of your items.",
          visual: <SupplierProductsVisual />,
        }
      : {
          id: "configurator",
          icon: StackIcon,
        ...
        },
    {
      ...
      title: supplier
        ? "Acknowledge and track purchase orders"
        : "Accept quotes and follow every order",
      body: supplier
        ? "Acknowledge purchase orders, confirm delivery dates and see payment runs, with a conversation attached to each one."
        : "Accept a quote online and it becomes an order. Track it from confirmation to delivery, request changes and message Brewfitt about it.",
    ...
    },
    {
      ...
      title: supplier
        ? "See what Brewfitt is likely to need"
        : "Know what to reorder before you run out",
      body: supplier
        ? "Insights flag low stock of your items and forecast demand. Ask the assistant about any purchase order or product. In this preview insights come from fixed rules on demonstration data."
        : "Insights flag regular items that are due, stock running low and quotes about to expire. Ask the assistant about any order or product. In this preview insights come from fixed rules on demonstration data.",
    ...
    },
    {
      ...
      body: "Your dashboard is next. You can take this tour again from the user menu at any time.",
      visual: <DoneVisual />,
    },
  ];
}
...
function Panel({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <div className={cn("rounded-3xl border bg-card p-5 shadow-sm sm:p-6", className)}>
      {children}
    </div>
  );
}
    ...
    <Panel>
      <div className="grid grid-cols-2 gap-3">
        ...
        <Kpi
          ...
          value={String(
            (orders.data ?? []).filter((o) => OPEN_ORDER_STATUSES.includes(o.status)).length,
          )}
        />
        {credit?.onAccount && credit.available && credit.limit ? (
          <Kpi
            label="Credit available"
            value={formatMoney(credit.available, { whole: true })}
            detail={`of ${formatMoney(credit.limit, { whole: true })}`}
          />
        ) : (
          <Kpi label="Payment" value="By card" detail="At checkout and on invoices" />
        )}
        <Kpi
          label="Unread conversations"
          value={String((threads.data ?? []).filter((t) => t.unreadCount > 0).length)}
        />
      </div>
    </Panel>
  );
    ...
    <Panel>
      <div className="grid grid-cols-2 gap-3">
        <Kpi
          ...
          value={String((rfqs.data ?? []).filter((r) => r.status === "open").length)}
        />
        <Kpi
          label="Open purchase orders"
          value={String((pos.data ?? []).filter((p) => OPEN_PO_STATUSES.includes(p.status)).length)}
        />
        <Kpi
          label="Owed by Brewfitt"
          value={
            statement.data ? formatMoney(statement.data.closingBalance, { whole: true }) : "None"
          }
        />
        <Kpi
          label="Next payment run"
          value={next ? formatMoney(next.total, { whole: true }) : "None scheduled"}
          detail={next ? formatDate(next.scheduledFor) : undefined}
        />
      </div>
    </Panel>
  );
...
function ShopVisual() {
  ...
  const picks = (priceList.data?.lines ?? [])
    .filter((l) => l.product.images.length)
    .filter((_, i) => i % 23 === 0)
    .slice(0, 4);
  return (
    <Panel>
...
function OrderVisual() {
  ...
  const order =
    sorted.find((o) => o.status === "dispatched" || o.status === "picking") ??
    sorted.find((o) => OPEN_ORDER_STATUSES.includes(o.status)) ??
    sorted[0];
  if (!order)
    return (
      <Panel>Your orders will appear here with every stage from confirmation to delivery.</Panel>
    );
  return (
    <Panel>
      ...
      <OrderStageTracker
        stages={salesOrderStages(
          order,
          (deliveries.data ?? []).filter((d) => d.orderId === order.id),
        )}
...
function PurchaseOrderVisual() {
  ...
  const po =
    [...(pos.data ?? [])]
      ...
      .find((p) => OPEN_PO_STATUSES.includes(p.status)) ?? pos.data?.[0];
    ...
    <Panel>
      ...
      <OrderStageTracker
        stages={purchaseOrderStages(
          po,
          (deliveries.data ?? []).filter((d) => d.orderId === po.id),
        )}
      />
    </Panel>
  );
...
function InsightVisual() {
  ...
  if (!insight)
    return (
      <Panel>
        No insights right now. They appear when a regular item is due or stock runs low.
      </Panel>
    );
  ...
  return (
    <Panel className="flex flex-col items-center py-10 text-center">
      <motion.span
        ...
        className="flex size-16 items-center justify-center rounded-full bg-success-subtle text-success"
      >
        <CheckCircleIcon weight="fill" className="size-9" aria-hidden />
      </motion.span>
      <p className="mt-4 font-medium">Tour complete</p>
      <p className="mt-1 text-sm text-muted-foreground">
        Go back to any step with Back or the arrow keys.
      </p>
    </Panel>
  );
}
