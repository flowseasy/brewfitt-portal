...
function ThreadPageInner() {
  ...
  const thread = useQuery({
    queryKey: queryKeys.thread(key, id),
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-[340px_minmax(0,1fr)]">
      <aside className="hidden lg:block">
        ...
        <div className="max-h-[calc(100dvh-12rem)] overflow-y-auto rounded-2xl">
          {threads.isPending ? (
            <LoadingState rows={6} />
          ) : threads.data ? (
            <ThreadList threads={threads.data} activeId={id} dense />
          ) : null}
        </div>
      </aside>
          ...
          <div className="rounded-2xl border bg-card p-4 sm:p-5">
            <header className="mb-4 flex flex-col gap-3 border-b pb-4 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div className="mt-2 flex items-center gap-2">
                  ...
                  <span className="text-sm text-muted-foreground">
                    {thread.data.participants
                      .map((p) => (p.side === "brewfitt" ? `${p.name} (Brewfitt)` : p.name))
                      .join(", ")}
                  </span>
                </div>
              </div>
              {thread.data.relatedType && thread.data.relatedId ? (
                <Link
                  href={hrefFor(thread.data.relatedType, thread.data.relatedId)}
                  className="inline-flex shrink-0 items-center gap-1.5 rounded-full border px-3 py-1.5 text-sm hover:border-primary/40"
                >
