...
export default function PriceListPage() {
  ...
  const exportCsv = useMutation({
    ...
    onError: (error) =>
      toast.error("The download could not be prepared", { description: errorMessage(error) }),
  });
    ...
    () =>
      priceList.data
        ? filterCatalogue(priceList.data.lines, cats, {
            ...
            section,
            category,
            inStockOnly: stockFilter === "available",
            sort: "name",
          })
        : [],
  ...
  const tree = useMemo(
    () => categoryTree(cats).filter((t) => section === "all" || t.category.section === section),
    [cats, section],
  );
  const active = [search, section !== "all", category !== "all", stockFilter !== "all"].filter(
    Boolean,
  ).length;
  const clear = () => {
    setSearch("");
  ...
  return (
    <div>
      <PageHeader
        ...
        description={
          list
            ? `${list.name}, valid from ${formatDate(list.validFrom)}. Prices exclude VAT.`
            : "The prices that apply to your account."
        }
        actions={
          <>
          ...
          <>
            <FilterSelect
              ...
              options={[
                ...
                ...(Object.keys(SECTION_LABEL) as CatalogueSection[]).map((s) => ({
                  value: s,
                ...
                {filtered.slice(0, limit).map((l) => (
                  <TableRow key={l.productId}>
                      ...
                      >
                        ...
                        <span className="min-w-0">
                          ...
                          <span className="block truncate text-xs text-muted-foreground">
                            {categoryName.get(l.product.subcategory ?? l.product.category)}
                          </span>
                        </span>
                      </Link>
                    </TableCell>
                    ...
                    <TableCell className="text-muted-foreground">
                      {l.product.packSize > 1
                        ? `Pack of ${l.product.packSize}`
                        : l.product.unit === "metre"
                          ? "Per metre"
                          : "Each"}
                    </TableCell>
                    ...
                    <TableCell className="pr-4 text-right">
                      <Button
                        ...
                        disabled={add.isPending || l.stock?.status === "out"}
      ...
      {priceList.data ? (
        <PdfPreview open={pdfOpen} onOpenChange={setPdfOpen} title="Price list PDF">
          <PriceListDocument
            lines={priceList.data.lines}
            name={priceList.data.priceList.name}
            validFrom={priceList.data.priceList.validFrom}
      ...
      {priceList.data && !priceList.isPending && priceList.data.lines.length === 0 ? (
        <EmptyState
          icon={TagIcon}
          title="No price list is assigned yet"
          description="Ask your Brewfitt account manager to set up your trade prices."
        />
      ) : null}
    </div>
  );
  ...
  return (
    <>
      <DocumentHeader
        ...
        meta={[{ label: "Valid from", value: formatDate(validFrom) }]}
      />
      <div className="my-6">
              ...
              <tbody>
                {items.map((l) => (
                  <tr key={l.productId} className="border-t border-neutral-100">
                    ...
                    <td className="py-1 text-right whitespace-nowrap text-neutral-500">
                      {formatMoney(l.product.listPrice)}
                    </td>
