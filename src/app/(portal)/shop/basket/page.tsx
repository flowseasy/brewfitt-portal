...
export default function BasketPage() {
  ...
  const lines = useMemo(() => {
    ...
    return (basket.data?.lines ?? []).map((l) => ({ ...l, line: byId.get(l.productId) })).filter((l) => !!l.line);
  ...
  const form = useForm<CheckoutValues>({
    ...
    values: {
      deliveryAddressId: basket.data?.deliveryAddressId ?? siteAddresses.find((a) => a.isDefault)?.id ?? "",
  ...
  const checkout = useMutation({
    ...
    onSuccess: (order) => {
      setPlaced(order);
      for (const k of [queryKeys.basket(key), queryKeys.salesOrders(key), queryKeys.invoices(key), queryKeys.statement(key), queryKeys.priceList(key), queryKeys.stock(key), queryKeys.notifications(key), queryKeys.threads(key), queryKeys.me(key), queryKeys.payments(key)]) {
        void queryClient.invalidateQueries({ queryKey: k });
      }
      window.scrollTo({ top: 0, behavior: "smooth" });
    },
    onError: (error) => toast.error("Your order could not be placed", { description: errorMessage(error) }),
  });

  if (placed) {
    return (
      <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="mx-auto max-w-xl rounded-3xl border bg-card p-8 text-center" role="status">
        <CheckCircleIcon weight="fill" className="mx-auto size-14 text-success" aria-hidden />
        <h1 className="mt-4 text-2xl font-semibold tracking-tight">Order {placed.number} confirmed</h1>
        <p className="mt-2 text-muted-foreground">
          {onAccount ? "Charged to your account." : "Paid by card."} Delivery requested for {formatDate(placed.requestedDate)}. Brewfitt has emailed a confirmation, and you can follow the order here.
        </p>
  ...
  if (basket.isPending || priceList.isPending || me.isPending) return <LoadingState rows={4} label="Loading your basket" />;
  if (basket.isError) return <ErrorState error={basket.error} onRetry={() => basket.refetch()} />;

  return (
    <div>
      <PageHeader title="Basket" description={lines.length ? `${plural(lines.length, "product")} at your price-list prices.` : undefined} actions={<Button asChild variant="outline"><Link href="/shop">Continue shopping</Link></Button>} />

      {lines.length === 0 ? (
        <EmptyState icon={ShoppingCartSimpleIcon} title="Your basket is empty" description="Add stock items from the shop or reorder your regular products." action={<Button asChild><Link href="/shop?reorder=1">Buy again</Link></Button>} />
      ) : (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)]">
            ...
            <ul className="divide-y rounded-2xl border bg-card">
              {lines.map(({ productId, qty, line }) => (
                <li key={productId} className="flex gap-3 p-4">
                  <ProductImage src={line!.product.images[0]} alt="" sizes="72px" className="size-16 shrink-0 rounded-xl border sm:size-20" />
                  <div className="min-w-0 flex-1">
                    <Link href={hrefFor("product", productId)} className="line-clamp-2 text-sm font-medium hover:underline">
                      {line!.product.name}
                    </Link>
                    <p className="font-mono text-[11px] text-muted-foreground">{line!.product.sku}</p>
                    <div className="mt-1 flex flex-wrap items-center gap-2">
                      <StockPill stock={line!.stock} />
                      <span className="text-xs text-muted-foreground tabular-nums">{formatMoney(line!.price)} each</span>
                    </div>
                    <div className="mt-3 flex items-center justify-between gap-2">
                      <QuantityStepper
                        ...
                        onChange={(n) => update.mutate({ lines: basket.data!.lines.map((l) => (l.productId === productId ? { ...l, qty: n } : l)) })}
                      />
                      <Button variant="ghost" size="sm" onClick={() => update.mutate({ lines: basket.data!.lines.filter((l) => l.productId !== productId) })} aria-label={`Remove ${line!.product.name}`}>
                        <TrashIcon aria-hidden />
                        <span className="hidden sm:inline">Remove</span>
                      </Button>
                    </div>
                  </div>
                  <p className="shrink-0 text-right font-medium tabular-nums">{formatMoney({ amount: qty * line!.price.amount, currency: "GBP" })}</p>
                </li>
              ))}
            </ul>
          </section>
          ...
          <section aria-labelledby="checkout" className="lg:sticky lg:top-20 lg:self-start">
            ...
            >
              ...
              {groupRollUp ? (
                <p className="mb-4 rounded-xl bg-info-subtle p-3 text-sm">Choose a site with the site switcher to place this order for it.</p>
              ) : null}
              <FieldGroup>
                <Controller
                  ...
                  render={({ field, fieldState }) => (
                    <FieldSet data-invalid={fieldState.invalid || undefined}>
                      ...
                      <div className="space-y-2">
                        {siteAddresses.map((a) => (
                          <label key={a.id} className="flex cursor-pointer gap-3 rounded-xl border p-3 has-checked:border-primary has-checked:bg-brand-subtle/50">
                            <input type="radio" name={field.name} value={a.id} checked={field.value === a.id} onChange={() => field.onChange(a.id)} className="mt-1 accent-[var(--primary)]" />
                            <span className="min-w-0 text-sm">
                              <span className="block font-medium">{a.label}</span>
                              <span className="block text-muted-foreground">{formatAddress(a).join(", ")}</span>
                              {a.deliveryNotes ? <span className="mt-0.5 block text-xs text-muted-foreground">{a.deliveryNotes}</span> : null}
                            </span>
                          </label>
                        ))}
                      </div>
                ...
                />
                <TextField control={form.control} name="requestedDate" label="Requested delivery date" type="date" min={earliest()} description="Monday to Friday. Brewfitt confirms the date." />
                <TextField control={form.control} name="poReference" label="Your PO reference (optional)" nullable />
              ...
              <dl className="mt-5 space-y-1.5 border-t pt-4 text-sm">
                <div className="flex justify-between">
                  ...
                  <dd className="tabular-nums">{formatMoney({ amount: subtotal, currency: "GBP" })}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-muted-foreground">VAT {vatRate === 0 ? "(export, zero-rated)" : `at ${Math.round(vatRate * 100)}%`}</dt>
                  <dd className="tabular-nums">{formatMoney({ amount: vat, currency: "GBP" })}</dd>
                </div>
                <div className="flex justify-between text-base font-semibold">
                  <dt>Total</dt>
                  <dd className="tabular-nums">{formatMoney({ amount: total, currency: "GBP" })}</dd>
                </div>
              </dl>
              ...
              {onAccount && available !== undefined && total > available ? (
                <p className="mt-3 flex items-start gap-2 rounded-xl bg-warning-subtle p-3 text-sm">
                  ...
                  This order is more than your available credit of {formatMoney({ amount: available, currency: "GBP" })}. Brewfitt credit control will be in touch before it is released.
                </p>
              ) : null}

              <Button type="submit" size="lg" className="mt-5 h-11 w-full rounded-full" disabled={checkout.isPending || groupRollUp}>
                {checkout.isPending ? "Placing order…" : onAccount ? "Place order on account" : `Pay ${formatMoney({ amount: total, currency: "GBP" })} and place order`}
              </Button>
              <p className="mt-2 text-center text-xs text-muted-foreground">Orders are subject to Brewfitt&apos;s conditions of sale.</p>
            </form>
          </section>
        </div>
      )}
