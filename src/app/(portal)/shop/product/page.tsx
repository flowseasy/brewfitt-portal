...
function Product() {
  ...
  const insight = useQuery({
    queryKey: queryKeys.productInsight(key, id),
    queryFn: () => api.ai.productInsight(id),
    enabled: !!id,
  });
  ...
  const related = useMemo(() => {
    ...
    return priceList.data.lines
      .filter(
        (l) =>
          l.productId !== id &&
          (l.product.subcategory ?? l.product.category) ===
            (line.product.subcategory ?? line.product.category),
      )
      .slice(0, 4);
  }, [line, priceList.data, id]);
  const articles = (knowledge.data ?? []).filter((k) => k.productIds.includes(id));
  const history = (orders.data ?? []).filter(
    (o) => o.status !== "cancelled" && o.lines.some((l) => l.productId === id),
  );

  if (priceList.isPending || categories.isPending)
    return <LoadingState rows={4} label="Loading product" />;
  if (priceList.isError)
    return <ErrorState error={priceList.error} onRetry={() => priceList.refetch()} />;
  if (!line) {
    return (
      <EmptyState
        icon={PackageIcon}
        title="This product is not on your price list"
        description="It may have been discontinued, or it is not part of your account's range."
        action={
    ...
    <div>
        ...
        >
          ...
          {category ? (
            <>
              ...
              <Link
                href={`/shop?section=${category.section}&category=${category.id}`}
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            ...
            <div className="mt-3 flex gap-2" role="group" aria-label="Product images">
              {p.images.map((src, i) => (
                <button
                  ...
                  aria-label={`Image ${i + 1} of ${p.images.length}`}
        ...
        <div>
          ...
          {subcategory ? (
            <p className="mt-1 text-sm text-muted-foreground">{subcategory.name}</p>
          ) : null}
          ...
          <div className="mt-5 rounded-2xl border bg-card p-5">
            <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
              ...
              {line.discountPercent > 0 ? (
                <span className="text-sm text-muted-foreground">
                  <span className="line-through">{formatMoney(p.listPrice)}</span> list ·{" "}
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2 text-sm">
              ...
              {line.stock && line.stock.status !== "out" && line.stock.available > 0 ? (
                <span className="text-muted-foreground">{line.stock.available} available</span>
              ) : null}
              {line.stock?.expectedAt ? (
                <span className="text-muted-foreground">
                  More expected {formatDate(line.stock.expectedAt)}
                </span>
              ) : null}
            ...
            <div className="mt-5 flex flex-wrap items-center gap-3">
              {out ? (
                <>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Out of stock. You can still order; Brewfitt will confirm the date.
                  </p>
                </>
              ) : null}
              ...
              <Button
                size="lg"
                className="h-10 flex-1 rounded-full sm:flex-none sm:px-6"
            ...
            {lastOrder ? (
              <p className="mt-4 text-sm text-muted-foreground">
                ...
                <Link
                  href={hrefFor("sales-order", lastOrder.id)}
          ...
          <section aria-labelledby="docs" className="mt-6 space-y-2">
            ...
            {p.specSheetDocumentId ? (
              <Link
                href={hrefFor("document", p.specSheetDocumentId)}
            ...
            {articles.map((a) => (
              <Link
                key={a.id}
                href={hrefFor("knowledge-item", a.id)}
                className="flex items-center gap-3 rounded-xl border bg-card p-3 hover:border-primary/40"
              >
