...
import { ArrowLeftIcon, CalendarCheckIcon, CreditCardIcon, FilePdfIcon, PackageIcon, ReceiptIcon } from "@phosphor-icons/react";
...
const KIND_LABEL = { invoice: "Invoice", "credit-note": "Credit note", "self-bill": "Self-billed invoice" } as const;
...
function InvoiceView({ id }: { id: string }) {
  ...
  const me = useMe();
  const invoice = useQuery({ queryKey: queryKeys.invoice(key, id), queryFn: () => api.invoices.get(id), enabled: !!id });
  ...
  const order = supplier ? pos.data?.find((p) => p.id === inv.orderId) : orders.data?.find((o) => o.id === inv.orderId);
  const lines = invoiceLines(inv, order, deliveries.data ?? []);
  const { net, vat } = splitVat(inv.total, lines);
  const allocations = (payments.data ?? []).flatMap((p) => p.allocatedTo.filter((a) => a.invoiceId === inv.id).map((a) => ({ payment: p, amount: a.amount })));
  const s = INVOICE_STATUS[inv.status];
  const canPay = !supplier && me.data?.credit?.onAccount === false && inv.kind === "invoice" && inv.outstanding.amount > 0;
  ...
  return (
    <div>
      ...
      {inv.status === "overdue" && !supplier ? (
        <div role="status" className="mb-5 rounded-2xl border border-danger/30 bg-danger-subtle p-4 text-sm">
          {formatMoney(inv.outstanding)} was due on {formatDate(inv.dueAt)}. {me.data?.credit?.onAccount ? "Please pay by bank transfer quoting the invoice number, or message credit control with any query." : "Pay by card to settle it."}
        </div>
      ) : null}
        ...
        <div className="min-w-0 space-y-6">
          <section aria-labelledby="inv-lines">
            ...
            {lines.length ? (
              <LinesTable
                ...
                products={products}
                lines={lines.map((l) => ({ productId: l.productId, description: products.get(l.productId)?.name ?? "Item", qty: l.qty, unitPrice: l.unitPrice, lineTotal: l.lineTotal, extra: inv.kind === "credit-note" ? "returned" : undefined }))}
              />
            ) : (
              <LoadingState rows={2} />
            )}
          ...
          <section aria-labelledby="inv-payments" className="rounded-2xl border bg-card p-5">
            ...
            {allocations.length === 0 ? (
              <p className="text-sm text-muted-foreground">{inv.kind === "credit-note" ? "Credit notes reduce your balance; see the statement." : supplier ? "Scheduled in a coming payment run." : "No payments received yet."}</p>
            ) : (
                ...
                {allocations.map(({ payment, amount }) => (
                  <li key={payment.id} className="flex items-center justify-between gap-3 py-2 text-sm">
                    <Link href={hrefFor("payment", payment.id)} className="hover:underline">
                      ...
                      <span className="block text-xs text-muted-foreground">{formatDate(payment.paidAt)}</span>
                    </Link>
        ...
        <aside className="space-y-4 lg:sticky lg:top-20 lg:self-start">
          <section aria-label="Amounts" className="rounded-2xl border bg-card p-5">
            ...
            <div className="mt-3 flex justify-between border-t pt-3 text-sm">
              <span className="text-muted-foreground">{inv.kind === "credit-note" ? "Unallocated" : "Outstanding"}</span>
          ...
          {order ? (
            <Link href={hrefFor(inv.orderType === "sales" ? "sales-order" : "purchase-order", order.id)} className="flex items-center gap-3 rounded-2xl border bg-card p-4 hover:border-primary/40">
              <PackageIcon className="size-5 text-primary" aria-hidden />
              <span className="text-sm">
                <span className="block text-xs text-muted-foreground">{inv.orderType === "sales" ? "For order" : "For purchase order"}</span>
      ...
      <PdfPreview open={pdfOpen} onOpenChange={setPdfOpen} title={`${KIND_LABEL[inv.kind]} ${inv.number}`}>
        <DocumentHeader kind={KIND_LABEL[inv.kind]} number={inv.number} date={inv.issuedAt} meta={[...(inv.kind !== "credit-note" ? [{ label: "Due", value: formatDate(inv.dueAt) }] : []), ...(order ? [{ label: "Order", value: order.number }] : [])]} />
        <div className="my-6 grid grid-cols-2 gap-6">
          <DocumentParty label={supplier ? "Supplier" : "Invoice to"} lines={[accountName, ...(billing ? formatAddress(billing) : [])]} />
          {inv.kind === "self-bill" ? <DocumentParty label="Self-billing" lines={["Issued by Brewfitt Limited under the self-billing agreement."]} /> : null}
        </div>
        ...
        <DocumentFooter note={inv.kind === "credit-note" ? "This credit is applied to your account." : `Payment due by ${formatDate(inv.dueAt)}.`} />
      </PdfPreview>
...
function PaymentView({ id }: { id: string }) {
  ...
  return (
    <div>
      <Back supplier={supplier} />
      <PageHeader eyebrow={supplier ? "Remittance advice" : "Payment"} title={formatMoney(p.amount)} description={`${METHOD[p.method]} · ${formatDateTime(p.paidAt)} · ${p.reference}`} />
      <section className="rounded-2xl border bg-card p-5" aria-labelledby="alloc">
        ...
        {p.remittanceDocumentId ? (
          <Link href={hrefFor("document", p.remittanceDocumentId)} className="mt-4 inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-sm hover:border-primary/40">
            <FilePdfIcon aria-hidden />
            Remittance advice
          </Link>
        ) : null}
...
function PaymentRunView({ id }: { id: string }) {
  ...
  if (!run) return <EmptyState icon={CalendarCheckIcon} title="This payment run could not be found" />;
    ...
    <div>
      ...
      <PageHeader
        ...
        title={
          <span className="flex flex-wrap items-center gap-3">
            ...
            <StatusPill tone={run.status === "paid" ? "success" : "info"}>{run.status === "paid" ? "Paid" : "Scheduled"}</StatusPill>
          </span>
        }
      ...
      <ul className="space-y-2">
        {run.invoiceIds.map((invoiceId) => {
          ...
          return (
            <li key={invoiceId}>
              <Link href={hrefFor("invoice", invoiceId)} className="flex items-center justify-between gap-3 rounded-2xl border bg-card p-4 hover:border-primary/40">
                <span>
                  <span className="block font-medium">{inv?.number ?? "Invoice"}</span>
                  {inv ? <span className="block text-sm text-muted-foreground">Due {formatDate(inv.dueAt)}</span> : null}
                </span>
                <span className="font-semibold tabular-nums">{inv ? formatMoney(inv.total) : ""}</span>
              </Link>
            </li>
          );
        })}
