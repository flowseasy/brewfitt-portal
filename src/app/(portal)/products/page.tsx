...
} from "@/components/supplier/submission-forms";
...
function Products() {
  ...
  const offers = useQuery({
    queryKey: queryKeys.offers(key),
    queryFn: () => api.supplierProducts.offers(),
  });
  ...
  const range = useMemo(
    () => (priceList.data?.lines ?? []).map((l) => l.product),
    [priceList.data],
  );
      ...
      {tab === "submissions" ? (
        submissions.isPending ? (
          ...
          <EmptyState
            ...
            action={<Button onClick={() => setProductOpen(true)}>Submit a product</Button>}
          />
        ) : (
          <ul className="space-y-2">
            {submissions.data.map((sp) => {
              ...
              return (
                <li key={sp.id}>
                  <Link
                    href={hrefFor("supplier-product", sp.id)}
                    className="flex items-center gap-3 rounded-2xl border bg-card p-3 hover:border-primary/40 sm:p-4"
                  >
                    <SubmissionImage
                      src={sp.images[0]}
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="truncate text-sm text-muted-foreground">
                        ...
                        {categoryName.get(sp.category) ?? "Category"} · cost{" "}
                        {formatMoney(sp.costPrice)} ·{" "}
                        {sp.specSheetDocumentId ? "spec sheet attached" : "no spec sheet"}
                      </p>
                      {sp.status === "rejected" && sp.reviewNote ? (
                        <p className="mt-1 text-sm text-danger">{sp.reviewNote}</p>
                      ) : null}
                    </div>
        ...
        offers.isPending ? (
          ...
          <EmptyState
            icon={TicketIcon}
            title="No offers yet"
            action={<Button onClick={() => setOfferOpen(true)}>Make an offer</Button>}
          />
        ) : (
          <ul className="space-y-2">
            {offers.data.map((o) => {
              const status =
                o.status === "approved" && daysFromToday(o.validTo) < 0 ? "expired" : o.status;
              const s = OFFER_STATUS[status];
              return (
                <li key={o.id}>
                  <Link
                    href={hrefFor("offer", o.id)}
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="truncate text-sm text-muted-foreground">
                        {o.productIds.map((id) => productName.get(id) ?? "Product").join(", ")} ·{" "}
                        {formatDate(o.validFrom)} to {formatDate(o.validTo)}
                      </p>
                    </div>
        ...
        <>
          <SearchInput
            value={search}
            onChange={setSearch}
            placeholder="Search your range"
            label="Search your range"
            className="mb-4 max-w-md"
          />
          <div className="overflow-x-auto rounded-2xl border bg-card">
            <table className="w-full min-w-[640px] text-sm">
              <caption className="sr-only">
                Products you supply to Brewfitt with agreed cost prices
              </caption>
                ...
                {priceList.data.lines
                  ...
                  .map((l) => (
                    <tr key={l.productId}>
                      ...
                      <td className="px-3 py-2.5 text-right tabular-nums">
                        {plural(l.product.leadTimeDays, "day")}
                      </td>
                      <td className="px-3 py-2.5">
                        {l.product.specSheetDocumentId ? (
                          <StatusPill tone="success">On file</StatusPill>
                        ) : (
                          <span className="text-muted-foreground">Not yet provided</span>
                        )}
                      </td>
                      <td className="px-4 py-2.5">
