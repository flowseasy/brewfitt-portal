...
export const quotes: PortalApi["quotes"] = {
  ...
  accept: (id) =>
    respond((db, scope) => {
      ...
      if (quote.status === "expired")
        badRequest(
          `Quote ${quote.number} expired on ${formatDate(quote.validUntil)}. Ask Brewfitt to reissue it.`,
        );
      if (quote.status !== "sent")
        badRequest(`Quote ${quote.number} is ${quote.status} and cannot be accepted.`);
      const at = nowIso();
  ...
  decline: (id, input) =>
    respond((db, scope) => {
      ...
      if (quote.status !== "sent")
        badRequest(`Quote ${quote.number} is ${quote.status} and cannot be declined.`);
      const at = nowIso();
      const message = postChanges(
        db,
        scope,
        quote.threadId,
        `We are declining ${quote.number}. ${reason}`,
        at,
      );
      commit("quotes.decline", [
        ...message.changes,
  ...
  respondToRfq: (id, input) =>
    respond((db, scope) => {
      ...
      const rfq =
        db.rfqs.find((r) => r.id === id && r.supplierId === scope.account.id) ??
        notFound("Request for quotation");
      if (rfq.status !== "open")
        badRequest(`${rfq.number} is ${rfq.status} and no longer accepts responses.`);
      if (daysBetween(today(), rfq.deadline) < 0)
        badRequest(`The deadline for ${rfq.number} was ${formatDate(rfq.deadline)}.`);
      const lineIds = new Set(rfq.lines.map((l) => l.productId));
      if (
        data.lines.some((l) => !lineIds.has(l.productId)) ||
        data.lines.length !== rfq.lines.length
      )
        badRequest("Price every line on the request.");
      const at = nowIso();
      const response: SupplierQuote = {
        id: newId("sq"),
        rfqId: rfq.id,
        lines: data.lines,
        notes: data.notes,
      ...
      const message = postChanges(
        ...
        scope,
        rfq.threadId,
        `Quote submitted for ${rfq.number}: ${data.lines.length} ${data.lines.length === 1 ? "line" : "lines"} priced.${data.notes ? ` ${data.notes}` : ""}`,
        at,
      );
      commit("quotes.respondToRfq", [
        insert("supplierQuotes", response),
        patch("rfqs", rfq.id, { status: "responded" }),
        ...message.changes,
      ]);
      return response;
    }),
...
export const orders: PortalApi["orders"] = {
  salesOrders: () =>
    respond((db, scope) => {
      ...
      return db.salesOrders
        .filter((o) => inScope(scope, o.accountId))
        .sort((a, b) => b.createdAt.localeCompare(a.createdAt));
    }),
  ...
  salesOrder: (id) =>
    respond((db, scope) => {
      const order =
        db.salesOrders.find((o) => o.id === id && inScope(scope, o.accountId)) ?? notFound("Order");
      return {
        ...order,
        deliveries: db.deliveries
          .filter((d) => d.orderType === "sales" && d.orderId === id)
  ...
  requestChange: (id, input) =>
    respond((db, scope) => {
      ...
      if (order.status !== "confirmed" && order.status !== "picking")
        badRequest(
          `Order ${order.number} has been ${order.status}; changes can only be requested before dispatch.`,
        );
      if (
        db.changeRequests.some(
          (c) => c.orderId === id && c.kind === data.kind && c.status === "pending",
        )
      )
        badRequest(
          `There is already a ${data.kind} change awaiting Brewfitt's approval on this order.`,
        );
      if (data.kind === "date" && daysBetween(today(), data.requested) < 1)
        badRequest("Choose a delivery date from tomorrow onwards.");
      if (
        data.kind === "address" &&
        !db.addresses.some((a) => a.id === data.requested && a.accountId === order.accountId)
      )
      ...
      const describe =
        data.kind === "date"
          ? `the delivery date to ${formatDate(data.requested)}`
          : `the delivery address to ${db.addresses.find((a) => a.id === data.requested)!.label}`;
      const message = postChanges(
        db,
        scope,
        order.threadId,
        `Change requested: please move ${describe}.${data.reason ? ` ${data.reason}` : ""}`,
        at,
      );
  ...
  purchaseOrder: (id) =>
    respond((db, scope) => {
      requireSupplier(scope);
      const po =
        db.purchaseOrders.find((p) => p.id === id && p.supplierId === scope.account.id) ??
        notFound("Purchase order");
      return {
        ...po,
        deliveries: db.deliveries.filter((d) => d.orderType === "purchase" && d.orderId === id),
      };
    }),
  ...
  acknowledge: (id) =>
    respond((db, scope) => {
      ...
      const po =
        ...
        notFound("Purchase order");
      if (po.status !== "issued")
        badRequest(`${po.number} has already been ${po.status.replace("-", " ")}.`);
      const at = nowIso();
      const message = postChanges(
        db,
        scope,
        po.threadId,
        `${po.number} acknowledged. Delivery confirmed for ${formatDate(po.expectedDate)}.`,
        at,
      );
...
export const deliveries: PortalApi["deliveries"] = {
  list: () =>
    respond((db, scope) => {
      if (scope.isSupplier) {
        const pos = new Set(
          db.purchaseOrders.filter((p) => p.supplierId === scope.account.id).map((p) => p.id),
        );
        return db.deliveries.filter((d) => d.orderType === "purchase" && pos.has(d.orderId));
      }
      const sos = new Set(
        db.salesOrders.filter((o) => inScope(scope, o.accountId)).map((o) => o.id),
      );
  ...
  get: (id) =>
    respond((db, scope) => {
      ...
      const owner =
        d.orderType === "sales"
          ? db.salesOrders.find((o) => o.id === d.orderId)?.accountId
          : db.purchaseOrders.find((p) => p.id === d.orderId)?.supplierId;
...
export const invoices: PortalApi["invoices"] = {
  list: () =>
    respond((db, scope) =>
      db.invoices
        .filter((i) => inScope(scope, i.accountId) && i.kind !== "credit-note")
        .map(invoiceNow)
  ...
  get: (id) =>
    respond((db, scope) =>
      invoiceNow(
        db.invoices.find((i) => i.id === id && inScope(scope, i.accountId)) ?? notFound("Invoice"),
      ),
    ),
  ...
  statement: () =>
    respond((db, scope): Statement => {
      ...
      const cash = db.payments.filter(
        (p) => inScope(scope, p.accountId) && p.method !== "credit-allocation",
      );
        ...
        BANDS.map((b) => [
          ...
          gbp(
            owed
              ...
              .reduce((sum, i) => sum + i.outstanding.amount, 0),
          ),
        ]),
      ) as Statement["ageing"];
      const unallocatedCredit = docs
        .filter((i) => i.kind === "credit-note")
        .reduce((sum, i) => sum + i.outstanding.amount, 0);
      const overdue = docs
  ...
  pay: (id, input) =>
    respond((db, scope) => {
      ...
      if (account(db, invoice.accountId).onAccount)
        forbidden(
          "Invoices on credit terms are paid by bank transfer to Brewfitt. Card payment is for accounts without credit terms.",
        );
      if (invoice.kind !== "invoice" || invoice.outstanding.amount <= 0)
        badRequest(`${invoice.number} has nothing to pay.`);
      const at = nowIso();
      const payment: Payment = {
        ...
        method: "card",
        reference: `Card ending ${card.last4}`,
        paidAt: at,
        allocatedTo: [{ invoiceId: invoice.id, amount: invoice.outstanding }],
        remittanceDocumentId: null,
      };
      commit("invoices.pay", [
        insert("payments", payment),
        patch("invoices", id, { outstanding: gbp(0), status: "paid" }),
      ]);
  ...
  payments: () =>
    respond((db, scope) =>
      db.payments
        .filter((p) => inScope(scope, p.accountId))
  ...
  paymentRuns: () =>
    respond((db, scope) => {
      requireSupplier(scope);
      const mine = new Map<string, Invoice>(
        db.invoices.filter((i) => i.accountId === scope.account.id).map((i) => [i.id, i]),
      );
