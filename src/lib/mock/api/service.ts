...
export const jobs: PortalApi["jobs"] = {
  list: () =>
    respond((db, scope) => {
      ...
      return db.jobs
        .filter((j) => inScope(scope, j.accountId))
        .sort((a, b) => b.scheduledDate.localeCompare(a.scheduledDate));
    }),
  get: (id) =>
    respond(
      (db, scope) =>
        db.jobs.find((j) => j.id === id && inScope(scope, j.accountId)) ?? notFound("Job"),
    ),
};
...
export const cases: PortalApi["cases"] = {
  ...
  get: (id) =>
    respond(
      (db, scope) =>
        db.cases.find((c) => c.id === id && inScope(scope, c.accountId)) ?? notFound("Case"),
    ),
  create: (input) =>
    respond((db, scope) => {
      requireCustomer(scope);
      ...
      if (
        ...
        notFound("Order");
      if (data.jobId && !db.jobs.some((j) => j.id === data.jobId && inScope(scope, j.accountId)))
        notFound("Job");
      ...
      if (data.status === "closed")
        changes.push(
          ...postChanges(
            ...
            scope,
            current.threadId,
            `Closing case ${current.number}; the issue is resolved for us.`,
            at,
          ).changes,
        );
...
const knowledgeVisible = (scope: Scope, k: KnowledgeItem) =>
  k.status === "approved" || (scope.isSupplier && k.submittedByAccountId === scope.account.id);

export const knowledge: PortalApi["knowledge"] = {
  list: () =>
    respond((db, scope) =>
      db.knowledge
        .filter((k) => knowledgeVisible(scope, k))
        .sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)),
    ),
  get: (id) =>
    respond(
      (db, scope) =>
        db.knowledge.find((k) => k.id === id && knowledgeVisible(scope, k)) ?? notFound("Article"),
    ),
  submit: (input) =>
    respond((db, scope) => {
      ...
      if (data.productIds.some((pid) => !db.products.some((p) => p.id === pid)))
        notFound("Product");
      const at = nowIso();
      ...
      if (data.fileName) {
        const doc: Document = {
          id: newId("doc"),
          name: data.fileName,
          category: data.type === "spec" ? "spec" : "manual",
          relatedType: "knowledge-item",
          relatedId: item.id,
          ownerAccountId: scope.account.id,
          fileType: "pdf",
          fileSize: 480_000,
          expiresAt: null,
...
export const supplierProducts: PortalApi["supplierProducts"] = {
  list: () =>
    respond((db, scope) => {
      requireSupplier(scope);
      return db.supplierProducts
        .filter((p) => p.supplierId === scope.account.id)
        .sort((a, b) => b.updatedAt.localeCompare(a.updatedAt));
    }),
  create: (input) =>
    respond((db, scope) => {
      ...
      const opened = openThreadChanges(db, scope, {
        ...
        subject: `Product submission: ${data.name}`,
        ...
        first: {
          side: "account",
          channel: "portal",
          body: `Submitted ${data.name} for review.`,
          at,
        },
      });
  ...
  update: (id, input) =>
    respond((db, scope) => {
      requireSupplier(scope);
      const current =
        db.supplierProducts.find((p) => p.id === id && p.supplierId === scope.account.id) ??
        notFound("Product submission");
      if (current.status === "approved" || current.status === "under-review")
        badRequest(
          `${current.name} is ${current.status.replace("-", " ")} and cannot be edited now.`,
        );
      ...
      if (specSheetFileName) {
        ...
        changes.push(
          ...
          patch("supplierProducts", id, { specSheetDocumentId: doc.id }),
        );
      }
      changes.push(
        ...postChanges(
          ...
          scope,
          current.threadId,
          `Resubmitted ${data.name ?? current.name} with updates.`,
          at,
        ).changes,
      );
  ...
  offers: () =>
    respond((db, scope) => {
      requireSupplier(scope);
      return db.offers
        .filter((o) => o.supplierId === scope.account.id)
  ...
  createOffer: (input) =>
    respond((db, scope) => {
      ...
      if (
        data.productIds.some(
          (pid) => db.products.find((p) => p.id === pid)?.supplierId !== scope.account.id,
        )
      )
        badRequest("Offers can only include products you supply to Brewfitt.");
      const offer: Offer = {
        ...data,
        id: newId("off"),
        supplierId: scope.account.id,
  ...
  performance: () =>
    respond((db, scope) => {
      ...
      const net = (po: (typeof db.purchaseOrders)[number]) =>
        po.lines.reduce((sum, l) => sum + l.qty * l.price.amount, 0);
      ...
      const topProducts = [...byProduct.entries()]
        ...
        .map(([productId, e]) => ({
          productId,
          quantity: e.quantity,
          total: { amount: e.total, currency: "GBP" as const },
        }));
      ...
      const totals = new Map<string, number>(
        db.accounts
          .filter((a) => a.kind === "supplier" && a.sector === sector)
          .map((a) => [a.id, 0]),
      );
      for (const po of recent)
        if (totals.has(po.supplierId))
          totals.set(po.supplierId, totals.get(po.supplierId)! + net(po));
      const ranked = [...totals.entries()].sort((a, b) => b[1] - a[1]);
      return {
        monthly,
        topProducts,
        last12Months: {
          amount: mine.reduce((sum, po) => sum + net(po), 0),
...
function documentVisible(db: MockDb, scope: Scope, d: Document): boolean {
  ...
  if (d.relatedType === "knowledge-item")
    return db.knowledge.some((k) => k.id === d.relatedId && knowledgeVisible(scope, k));
...
export const documents: PortalApi["documents"] = {
  list: () =>
    respond((db, scope) =>
      db.documents
        .filter((d) => documentVisible(db, scope, d))
        .sort((a, b) => b.modifiedAt.localeCompare(a.modifiedAt)),
    ),
  get: (id) =>
    respond(
      (db, scope) =>
        db.documents.find((d) => d.id === id && documentVisible(db, scope, d)) ??
        notFound("Document"),
    ),
};
...
export const messages: PortalApi["messages"] = {
      ...
      return db.threads
        ...
        .map((t) => {
          ...
          return {
            ...t,
            lastMessage: m
              ? { senderId: m.senderId, senderSide: m.senderSide, channel: m.channel, body: m.body }
              : null,
          };
        });
    }),
  thread: (id) =>
    respond((db, scope) => {
      const thread =
        db.threads.find((t) => t.id === id && inScope(scope, t.accountId)) ??
        notFound("Conversation");
      if (thread.unreadCount > 0)
        commit("messages.read", [patch("threads", id, { unreadCount: 0 })]);
      return {
        ...db.threads.find((t) => t.id === id)!,
  ...
  send: (threadId, input) =>
    respond((db, scope) => {
      ...
      if (!db.threads.some((t) => t.id === threadId && inScope(scope, t.accountId)))
        notFound("Conversation");
      const { message, changes } = postChanges(
        db,
        scope,
        threadId,
        data.body,
  ...
  createThread: (input) =>
    respond((db, scope) => {
      ...
      return {
        ...opened.thread,
        messages: db.messages.filter((m) => m.threadId === opened.thread.id),
      };
    }),
};
  ...
  list: () =>
    respond((db, scope) =>
      db.notifications
        .filter(
          (n) =>
            inScope(scope, n.accountId) && !n.dismissed && Date.parse(n.createdAt) <= Date.now(),
        )
        .sort((a, b) => b.createdAt.localeCompare(a.createdAt)),
    ),
  update: (id, input) =>
    respond((db, scope) => {
      ...
      if (!db.notifications.some((n) => n.id === id && inScope(scope, n.accountId)))
  ...
  const all = scope.isSupplier
    ...
    : orderingAccountIds(db, scope).flatMap((id) => {
        ...
        const suggestions = list
          .filter((i) => i.category === "product-suggestion")
...
export const ai: PortalApi["ai"] = {
  ...
  productInsight: (productId) =>
    respond(
      (db, scope) =>
        scopedInsights(db, scope).find(
          (i) => i.relatedType === "product" && i.relatedId === productId,
        ) ?? null,
    ),
  ask: (input) =>
    respond((db, scope) => {
      ...
      const listed = new Set(
        db.priceListLines.filter((l) => l.priceListId === listId).map((l) => l.productId),
      );
...
const PERSONA_CHOICES: {
  kind: PersonaOption["persona"]["kind"];
  accountId: string;
  contactName: string;
  description: string;
}[] = [
  {
    ...
    description: "Brand owner on account, running a Premium Lager font rollout",
  },
  {
    kind: "customer",
    accountId: "acc_pennine",
    contactName: "Sarah Crowther",
    description: "Brewery on account with regular consumables and a tap room",
  },
  {
    kind: "customer",
    accountId: "acc_crown",
    contactName: "Kerry Flanagan",
    description: "Independent pub with no credit terms: pays by card",
  },
  {
    kind: "customer",
    accountId: "acc_saltember",
    contactName: "Lucy Carrington",
    description: "London restaurant on account",
  },
  {
    kind: "customer",
    accountId: "acc_liffey",
    contactName: "Siobhan Kelly",
    description: "Export hotel in Dublin, invoiced without VAT",
  },
  {
    kind: "site",
    accountId: "acc_millrace_weavers",
    contactName: "Jess Armitage",
    description: "Site manager at one Mill Race Inns pub; sees their own site",
  },
  {
    kind: "group",
    accountId: "acc_millrace",
    contactName: "Andrew Hirst",
    description: "Mill Race Inns group contact; sees all sites and can switch into one",
  },
  {
    kind: "supplier",
    accountId: "sup_vireo",
    contactName: "Neil Chapman",
    description: "Font and tap manufacturer supplying Brewfitt",
  },
];
...
export const demo: PortalApi["demo"] = {
  personas: async () => {
    ...
    return PERSONA_CHOICES.map((choice) => {
      const contact =
        db.contacts.find(
          (c) => c.accountId === choice.accountId && c.name === choice.contactName,
        ) ?? notFound("Persona contact");
