...
export function seedContent(
  ...
  for (const q of records.quotes.filter((x) => x.status !== "draft")) {
    q.pdfDocumentId = addDoc({
      name: `Quote ${q.number}.pdf`,
      category: "quote",
      relatedType: "quote",
      relatedId: q.id,
      ownerAccountId: q.accountId,
      fileType: "pdf",
      fileSize: pdfSize(),
      expiresAt: null,
      approvalStatus: null,
      modifiedAt: q.createdAt,
    }).id;
  }
  for (const o of records.salesOrders.filter((x) => x.status !== "cancelled")) {
    addDoc({
      name: `Order confirmation ${o.number}.pdf`,
      category: "order",
      relatedType: "sales-order",
      relatedId: o.id,
      ownerAccountId: o.accountId,
      fileType: "pdf",
      fileSize: pdfSize(50, 180),
      expiresAt: null,
      approvalStatus: null,
  ...
  for (const po of records.purchaseOrders) {
    addDoc({
      name: `Purchase order ${po.number}.pdf`,
      category: "order",
      relatedType: "purchase-order",
      relatedId: po.id,
      ownerAccountId: po.supplierId,
      fileType: "pdf",
      fileSize: pdfSize(50, 160),
      expiresAt: null,
      approvalStatus: null,
      modifiedAt: po.createdAt,
    });
  }
  ...
  for (const d of records.deliveries) {
    ...
    if (d.dispatchedAt) {
      d.noteDocumentId = addDoc({
        name: `Delivery note ${d.number}.pdf`,
        category: "delivery-note",
        relatedType: "delivery",
        relatedId: d.id,
        ownerAccountId: owner,
        fileType: "pdf",
        fileSize: pdfSize(40, 120),
        expiresAt: null,
        approvalStatus: null,
        modifiedAt: d.dispatchedAt,
      }).id;
    }
    if (d.deliveredAt) {
      d.proofDocumentId = addDoc({
        name: `Proof of delivery ${d.number}.jpg`,
        category: "proof-of-delivery",
        relatedType: "delivery",
        relatedId: d.id,
        ownerAccountId: owner,
        fileType: "jpg",
        fileSize: pdfSize(180, 900),
        expiresAt: null,
        approvalStatus: null,
        modifiedAt: d.deliveredAt,
      }).id;
    }
  }
    ...
    const label =
      inv.kind === "credit-note"
        ...
        : inv.kind === "self-bill"
          ? "Self-billed invoice"
          : "Invoice";
    inv.pdfDocumentId = addDoc({
      name: `${label} ${inv.number}.pdf`,
      category: inv.kind === "credit-note" ? "credit-note" : "invoice",
      relatedType: "invoice",
      relatedId: inv.id,
      ownerAccountId: inv.accountId,
      fileType: "pdf",
      fileSize: pdfSize(45, 140),
      expiresAt: null,
      approvalStatus: null,
      modifiedAt: inv.issuedAt,
    }).id;
  }
  for (const p of records.payments.filter((x) => ctx.account(x.accountId).kind === "supplier")) {
    p.remittanceDocumentId = addDoc({
      name: `Remittance advice ${p.reference.replace("Payment run ", "")}.pdf`,
      category: "remittance",
      relatedType: "payment",
      relatedId: p.id,
      ownerAccountId: p.accountId,
      fileType: "pdf",
      fileSize: pdfSize(30, 90),
      expiresAt: null,
      approvalStatus: null,
      modifiedAt: p.paidAt,
    }).id;
  ...
  for (const [name, category, expiresInDays, ageDays] of companyDocs) {
    addDoc({
      ...
      ownerAccountId: null,
      fileType: "pdf",
      fileSize: pdfSize(90, 600),
      expiresAt: expiresInDays ? isoDate(addDays(today, expiresInDays)) : null,
      approvalStatus: null,
      modifiedAt: isoDateTime(addDays(today, -ageDays), 10),
    });
  }
  ...
  suppliers.forEach((s: Account, i) => {
    ...
    addDoc({
      name: `${s.name} public liability insurance certificate.pdf`,
      category: "insurance",
      relatedType: "account",
      relatedId: s.id,
      ownerAccountId: s.id,
      fileType: "pdf",
      fileSize: pdfSize(120, 500),
      expiresAt: isoDate(addDays(today, expiresIn)),
      approvalStatus: "approved",
      modifiedAt: isoDateTime(addDays(today, -(365 - Math.max(expiresIn, 0)) + rng.int(0, 20)), 11),
    });
    if (i % 3 === 0) {
      addDoc({
        name: `${s.name} product liability insurance certificate.pdf`,
        category: "insurance",
        relatedType: "account",
        relatedId: s.id,
        ownerAccountId: s.id,
        fileType: "pdf",
        fileSize: pdfSize(120, 500),
        expiresAt: isoDate(addDays(today, rng.int(90, 360))),
        approvalStatus: i === 3 ? "pending" : "approved",
        modifiedAt: isoDateTime(addDays(today, -rng.int(2, 40)), 11),
      });
    }
    if (i % 4 === 1) {
      addDoc({
        name: `${s.name} ISO 9001 certificate.pdf`,
        category: "compliance",
        relatedType: "account",
        relatedId: s.id,
        ownerAccountId: s.id,
        fileType: "pdf",
        fileSize: pdfSize(80, 300),
        expiresAt: isoDate(addDays(today, rng.int(120, 700))),
        approvalStatus: i === 1 ? "pending" : "approved",
        modifiedAt: isoDateTime(addDays(today, -rng.int(3, 200)), 11),
      });
    }
    addDoc({
      name: `Supply agreement: ${s.name}.pdf`,
      category: "agreement",
      relatedType: "account",
      relatedId: s.id,
      ownerAccountId: s.id,
      fileType: "pdf",
      fileSize: pdfSize(150, 450),
      expiresAt: isoDate(addDays(today, rng.int(120, 900))),
      approvalStatus: "approved",
      modifiedAt: isoDateTime(addDays(today, -rng.int(200, 700)), 10),
    });
    if (SELF_BILLING_SUPPLIERS.has(s.id)) {
      addDoc({
        name: `Self-billing agreement: ${s.name}.pdf`,
        category: "agreement",
        relatedType: "account",
        relatedId: s.id,
        ownerAccountId: s.id,
        fileType: "pdf",
        fileSize: pdfSize(80, 200),
        expiresAt: isoDate(addDays(today, rng.int(60, 400))),
        approvalStatus: "approved",
        modifiedAt: isoDateTime(addDays(today, -rng.int(100, 500)), 10),
      });
    }
  });

  // ---- Customer agreements --------------------------------------------------
  for (const a of ctx.accounts.filter(
    (x) => x.kind === "customer" && !x.parentAccountId && x.onAccount,
  )) {
    addDoc({
      name: `Trading terms agreement: ${a.name}.pdf`,
      category: "agreement",
      relatedType: "account",
      relatedId: a.id,
      ownerAccountId: a.id,
      fileType: "pdf",
      fileSize: pdfSize(120, 300),
      expiresAt: null,
      approvalStatus: "approved",
    ...
    if (a.sector === "brand-owner" || a.sector === "brewery") {
      addDoc({
        name: `Branded dispense equipment agreement: ${a.name}.pdf`,
        category: "agreement",
        relatedType: "account",
        relatedId: a.id,
        ownerAccountId: a.id,
        fileType: "pdf",
        fileSize: pdfSize(150, 380),
        expiresAt: isoDate(addDays(today, rng.int(90, 700))),
        approvalStatus: "approved",
        modifiedAt: isoDateTime(addDays(today, -rng.int(60, 400)), 10),
      });
    }
  }
  ...
  for (const sp of records.supplierProducts) {
    ...
    const doc = addDoc({
      name: `${sp.name} spec sheet.pdf`,
      category: "spec",
      relatedType: "supplier-product",
      relatedId: sp.id,
      ownerAccountId: sp.supplierId,
      fileType: "pdf",
      fileSize: pdfSize(150, 900),
      expiresAt: null,
      approvalStatus:
        sp.status === "approved" ? "approved" : sp.status === "rejected" ? "rejected" : "pending",
      modifiedAt: sp.submittedAt,
    });
    ...
    if (sp.status === "approved" && sp.productId) {
      ...
      product.specSheetDocumentId = addDoc({
        name: `${product.name} spec sheet.pdf`,
        category: "spec",
        relatedType: "product",
  ...
  const categoryFor = (name: string, productIds: string[]) => {
    ...
    const byProduct = matches.find((c) =>
      productIds.some(
        (pid) =>
          productById.get(pid)?.category === c.id || productById.get(pid)?.subcategory === c.id,
      ),
    );
  ...
  const knowledge: KnowledgeItem[] = (knowledgeRaw as KnowledgeRaw[]).map((k, i) => {
    ...
    if (productIds.length === 0)
      throw new Error(`Knowledge item ${k.key} references no known products`);
    ...
    if (k.fileType) {
      documentId = addDoc({
        ...
        expiresAt: null,
        approvalStatus:
          k.status === "approved" ? "approved" : k.status === "rejected" ? "rejected" : "pending",
