...
import { buildBillOfMaterials, checkCompatibility, type ConfigState } from "@/lib/configurator/engine";
...
const SELL_LISTS: { id: string; name: string; discount: (p: Product, roleIsConsumable: boolean, fontOrTap: boolean) => number; excludeLarge: boolean }[] = [
  ...
  { id: "pl_pubgroup", name: "Pub group price list", discount: (_p, consumable) => (consumable ? 12 : 10), excludeLarge: false },
  { id: "pl_brewery", name: "Brewery partner price list", discount: () => 12, excludeLarge: false },
  { id: "pl_brand", name: "Brand owner price list", discount: (_p, _c, fontOrTap) => (fontOrTap ? 15 : 10), excludeLarge: false },
  { id: "pl_export", name: "Export price list", discount: () => 8, excludeLarge: false },
];

const LARGE_EQUIPMENT = /Gamko Keg cooler .*(590|850|1200|1550|1840) litre|AS-160|AS-200|AS-450|CWP|Event Cooler|Water Cooled/i;
...
export function seedPriceLists(args: {
  ...
  for (const def of SELL_LISTS) {
    priceLists.push({ id: def.id, name: def.name, currency: "GBP", kind: "sell", validFrom, validTo: null });
  ...
  for (const s of suppliers) {
    ...
    priceLists.push({ id: s.priceListId, name: `Agreed cost prices: ${s.name}`, currency: "GBP", kind: "cost", validFrom, validTo: null });
    for (const p of products.filter((x) => x.supplierId === s.id)) {
      priceListLines.push({ priceListId: s.priceListId, productId: p.id, price: money(p.listPrice.amount * (0.5 + rng.next() * 0.12)), discountPercent: 0 });
    }
  }
...
const OVERDUE_ACCOUNTS = new Set(["acc_pennine", "acc_crown", "acc_millrace_weavers", "acc_fleece", "acc_kelpie"]);
...
export type OrderOrigin = { kind: "cycle" } | { kind: "quote"; quoteId: string } | { kind: "rollout"; quoteId: string };
...
const CONFIG_SPECS: ConfigSpec[] = [
  { accountId: "acc_harbourside", name: "Harbourside flagship bar fit-out", venueType: "bar", points: [["lager", "lager", "lager", "lager"], ["lager", "cider", "beer"]], metres: 22, prefer: ["o-tap-fc4-chrome", "o-coupler-s", "o-font-cobra-b-led", "o-badge-extended-chrome", "o-cooler-v21", "o-python-8", "o-cooltube-pancake", "o-coolant-dtx-premix", "o-gas-both", "o-gas-secondary", "o-gas-chain", "o-drip-freshener", "o-clean-s", "o-fob-stop"], quote: "sent", siteLabel: "Northern install store", branding: "Harbourside Premium Lager illuminated badges" },
  { accountId: "acc_harbourside", name: "Summer festival bar", venueType: "event", points: [["lager", "lager"], ["lager", "cider"]], metres: 0, prefer: ["o-tap-fc4-chrome", "o-coupler-s", "o-font-event", "o-cooler-kontakt-70k", "o-gas-co2", "o-drip-standard", "o-clean-s"], quote: "accepted", siteLabel: "Midlands install store", branding: "Harbourside Premium Lager" },
  { accountId: "acc_harbourside", name: "Hotel lobby bar trial", venueType: "hotel", points: [["lager", "lager"]], metres: 12, prefer: ["o-tap-celtic-chrome", "o-coupler-s", "o-font-cobra-pl-led", "o-cooler-v15", "o-python-2", "o-gas-co2", "o-gas-chain", "o-drip-recessed"], quote: "none", branding: "Harbourside Premium Lager" },
  { accountId: "acc_pennine", name: "Tap room back bar", venueType: "brewery-taproom", points: [["beer", "beer", "beer"], ["lager", "cider", "beer"]], metres: 9, prefer: ["o-tap-aceline-steel", "o-coupler-keykeg", "o-font-classic-cobra", "o-cooler-lindr-as200", "o-gas-both", "o-gas-secondary", "o-gas-chain", "o-drip-standard", "o-clean-keykeg"], quote: "accepted", siteLabel: "Brewery tap room" },
  { accountId: "acc_pennine", name: "Beer festival stand", venueType: "event", points: [["beer", "beer"]], metres: 0, prefer: ["o-tap-aceline-steel", "o-coupler-keykeg", "o-font-event", "o-cooler-kontakt-70k", "o-gas-mixed"], quote: "none" },
  { accountId: "acc_trent", name: "Trade partner starter system", venueType: "pub", points: [["lager", "beer"], ["beer", "cider"]], metres: 18, prefer: ["o-tap-fc4-chrome", "o-coupler-s", "o-font-cobra-pl-led", "o-badge-standard-chrome", "o-cooler-v21", "o-python-4", "o-coolant-4flow", "o-gas-both", "o-gas-secondary", "o-gas-chain", "o-drip-freshener", "o-clean-s", "o-fob-stop"], quote: "accepted", siteLabel: "Trade dispense store" },
  { accountId: "acc_millrace_weavers", name: "Garden bar extension", venueType: "pub", points: [["lager", "cider"]], metres: 25, prefer: ["o-tap-fc4-chrome", "o-coupler-s", "o-font-cobra-pl", "o-cooler-v15", "o-python-2", "o-cooltube-single", "o-gas-co2", "o-gas-chain", "o-drip-standard", "o-clean-s"], quote: "sent" },
  { accountId: "acc_millrace_plough", name: "Main bar refit", venueType: "pub", points: [["lager", "lager", "cider"], ["beer", "beer", "lager"]], metres: 14, prefer: ["o-tap-fc4-chrome", "o-coupler-s", "o-font-classic-cobra", "o-cooler-v21", "o-python-6", "o-gas-both", "o-gas-secondary", "o-gas-chain", "o-drip-freshener", "o-clean-s", "o-fob-stop"], quote: "draft" },
  { accountId: "acc_neontiger", name: "Cocktail bar draught station", venueType: "bar", points: [["lager", "lager"], ["soft", "water"]], metres: 8, prefer: ["o-tap-compensator", "o-coupler-s", "o-postmix", "o-soda-water", "o-font-cobra-pl-led", "o-cooler-lindr-as200", "o-gas-co2", "o-gas-chain", "o-drip-recessed"], quote: "accepted" },
  { accountId: "acc_saltember", name: "Restaurant bar counter", venueType: "restaurant", points: [["lager", "lager"], ["coffee"]], metres: 10, prefer: ["o-tap-celtic-chrome", "o-coupler-s", "o-hot-drinks", "o-font-cobra-pl-led", "o-cooler-v15", "o-python-2", "o-gas-co2", "o-gas-chain", "o-drip-recessed"], quote: "sent" },
  { accountId: "acc_coachworks", name: "Ballroom bar", venueType: "hotel", points: [["lager", "lager", "lager"], ["lager", "cider", "beer"]], metres: 30, prefer: ["o-tap-fc4-gold", "o-coupler-s", "o-font-classic-cobra", "o-badge-standard-gold", "o-cooler-v21", "o-python-6", "o-cooltube-pancake", "o-coolant-dtx-premix", "o-gas-both", "o-gas-secondary", "o-gas-chain", "o-drip-freshener"], quote: "draft" },
  { accountId: "acc_copperhouse", name: "Second taproom", venueType: "brewery-taproom", points: [["beer", "beer", "lager", "lager"]], metres: 6, prefer: ["o-tap-aceline-steel", "o-coupler-keykeg", "o-font-cobra-b-led", "o-cooler-v15", "o-python-4", "o-gas-both", "o-gas-secondary", "o-gas-chain", "o-drip-standard", "o-clean-keykeg"], quote: "none" },
  { accountId: "acc_crown", name: "Replacement fonts", venueType: "pub", points: [["lager", "lager"], ["beer", "cider"]], metres: 12, prefer: ["o-tap-fc4-chrome", "o-coupler-s", "o-font-cobra-pl", "o-cooler-v15", "o-python-4", "o-gas-both", "o-gas-chain", "o-drip-standard"], quote: "none" },
  { accountId: "acc_liffey", name: "Rooftop terrace bar", venueType: "hotel", points: [["lager", "beer"], ["lager", "cider"]], metres: 35, prefer: ["o-tap-celtic-chrome", "o-coupler-g", "o-font-cobra-pl-led", "o-cooler-v21", "o-python-4", "o-cooltube-single", "o-gas-both", "o-gas-secondary", "o-gas-chain", "o-drip-freshener", "o-clean-g"], quote: "sent" },
  { accountId: "acc_wildfell", name: "Seltzer launch events", venueType: "event", points: [["lager", "lager"]], metres: 0, prefer: ["o-tap-fc4-chrome", "o-coupler-keykeg", "o-font-event", "o-cooler-kontakt-70k", "o-gas-co2"], quote: "none" },
  { accountId: "acc_granitequay", name: "Hotel bar upgrade", venueType: "hotel", points: [["lager", "lager", "cider"]], metres: 16, prefer: ["o-tap-fc4-chrome", "o-coupler-s", "o-font-classic-cobra", "o-cooler-v15", "o-python-4", "o-gas-co2", "o-gas-secondary", "o-gas-chain", "o-drip-standard", "o-clean-s"], quote: "none" },
];
...
function buildSelections(spec: ConfigSpec, siteAddressId: string): { state: ConfigState } {
  ...
  const selections: ConfigurationSelections = {
    venue: { newSite: null },
    dispense: { points: spec.points.map((taps, i) => ({ id: `pt-${i + 1}`, name: `Dispense point ${i + 1}`, taps })), optionIds: [] },
  ...
  const bucket = (stepId: string) =>
    stepId === "dispense" ? selections.dispense : stepId === "font" ? selections.font : stepId === "cooling" ? selections.cooling : stepId === "gas" ? selections.gas : selections.ancillaries;
  ...
  for (const optionId of spec.prefer) {
    ...
    if (checkCompatibility(option, state, rules).compatible) bucket(group.stepId).optionIds.push(optionId);
  }
  ...
  for (const o of rules.options.filter((x) => x.groupId === "g-other-dispense")) {
    if (!selections.dispense.optionIds.includes(o.id) && checkCompatibility(o, state, rules).compatible) selections.dispense.optionIds.push(o.id);
  }
  return { state };
}
...
export function seedCommerce(ctx: SeedContext, people: { addresses: Address[] }): CommerceSeed {
  ...
  const makeQuote = (accountId: string, createdAt: Date, lineDrafts: LineDraft[], status: Quote["status"], configurationId: string | null): Quote => {
    ...
    const lines: QuoteLine[] = priced(accountId, lineDrafts).map((l) => {
      ...
      return {
        ...
        qty: l.qty,
        unitPrice: l.price,
        discountPercent: Math.max(0, Math.round((1 - l.price.amount / product.listPrice.amount) * 100)),
  ...
  const makeOrder = (accountId: string, createdAt: Date, lineDrafts: LineDraft[], opts: { quoteId?: string; addressId?: string; po?: string | null; leadDays?: number }): SalesOrder => {
    const created = previousWorkingDay(createdAt);
    const lines = priced(accountId, lineDrafts).map((l) => ({ productId: l.productId, qty: l.qty, delivered: 0, backordered: 0, price: l.price }));
  ...
  for (const account of customers) {
    ...
    const consumables = rng.sample(REGULAR_CONSUMABLES.map(slugId).filter((id) => available.has(id)), 2);
    const spares = rng.sample(REGULAR_SPARES.map(slugId).filter((id) => available.has(id)), rng.int(1, 2));
    ...
    let t = overdue ? addDays(today, -Math.round(interval * (1.3 + rng.next() * 0.4))) : addDays(today, -rng.int(2, Math.round(interval * 0.95)));
    const dates: Date[] = [];
    ...
    for (const d of dates.reverse()) {
      const lines: LineDraft[] = regular
        ...
        .map((productId) => ({ productId, qty: ctx.roleOf(ctx.productById.get(productId)!) === "consumable" ? rng.int(1, 4) : rng.int(1, 3) }));
      if (rng.chance(0.15)) {
        const extra = rng.pick(REGULAR_SPARES.map(slugId).filter((id) => available.has(id) && !regular.includes(id)));
        if (extra) lines.push({ productId: extra, qty: rng.int(1, 2) });
      }
      const order = makeOrder(account.id, d, lines, { po: account.onAccount && rng.chance(0.5) ? `${account.name.split(" ")[0]!.toUpperCase().slice(0, 4)}-${rng.int(1000, 9999)}` : null });
  ...
  CONFIG_SPECS.forEach((spec, i) => {
    ...
    const bom = buildBillOfMaterials(state, CONFIGURATOR_RULES, (pid) => ctx.priceFor(spec.accountId, pid));
    const createdAt = spec.quote === "accepted" ? ctx.seasonalDate(200, 45) : spec.quote === "sent" ? addDays(today, -rng.int(6, 20)) : addDays(today, -rng.int(3, 40));
    const id = `cfg_${i + 1}`;
    ...
    if (spec.quote !== "none") {
      ...
      const q = makeQuote(spec.accountId, quoteDate > today ? today : quoteDate, bom.lines.map((l) => ({ productId: l.productId, qty: l.qty })), spec.quote, id);
      quotes.push(q);
      config.quoteId = q.id;
  ...
  const quotesPerSector: Record<string, number> = { "brand-owner": 3, brewery: 2, hotel: 2, restaurant: 1, pub: 1 };
  const projectPool = (accountId: string) => ctx.listProducts(accountId).filter((p) => ctx.roleOf(p) === "project");
  const sparePool = (accountId: string) => ctx.listProducts(accountId).filter((p) => ctx.roleOf(p) === "spare");
  for (const account of customers) {
    const sector = account.parentAccountId ? "pub" : account.sector;
    const n = (quotesPerSector[sector] ?? 1) + (account.sector === "restaurant" && rng.chance(0.5) ? 1 : 0);
    for (let i = 0; i < n; i++) {
      const created = ctx.seasonalDate(360, 3);
      const age = daysBetween(created, today);
      const status: Quote["status"] =
        age <= 30 ? rng.weighted([["sent", 6], ["draft", 1.5], ["accepted", 2.5]] as const) : rng.weighted([["accepted", 5.5], ["declined", 2], ["expired", 2.5]] as const);
      const project = rng.sample(projectPool(account.id), rng.int(1, 2));
      const spares = rng.sample(sparePool(account.id), rng.int(1, 4));
      const lines: LineDraft[] = [
        ...project.map((p) => ({ productId: p.id, qty: p.listPrice.amount > 150_000 ? 1 : rng.int(1, 4) })),
  ...
  for (const accountId of ["acc_pennine", "acc_harbourside", "acc_coachworks"]) {
    ...
    const q = makeQuote(accountId, addDays(today, -rng.int(24, 27)), [...project.map((p) => ({ productId: p.id, qty: 2 })), ...spares.map((p) => ({ productId: p.id, qty: rng.int(4, 10) }))], "sent", null);
    quotes.push(q);
  }
  ...
  for (const q of quotes) {
    ...
    if (q.status === "sent") {
      ...
      q.lastViewedAt = age >= 23 ? null : rng.chance(0.7) ? isoDateTime(addDays(today, -rng.int(0, Math.min(age, 4))), rng.int(8, 18), 5) : null;
      ...
      const decided = q.id === rolloutQuote.id ? addDays(created, 4) : addDays(created, rng.int(1, Math.min(14, Math.max(1, daysBetween(created, today)))));
      ...
      const order = makeOrder(q.accountId, decidedAt, q.lines.map((l) => ({ productId: l.productId, qty: l.qty })), {
        quoteId: q.id,
        addressId: config?.siteAddressId ?? (isRollout ? install.find((a) => a.label === "Midlands install store")!.id : undefined),
        po: account.onAccount ? (isRollout ? "HD-PL-FONTS-Q3" : `PO${rng.int(20000, 89999)}`) : null,
        leadDays: rng.int(10, 21),
      });
      salesOrders.push(order);
      origins.set(order.id, isRollout ? { kind: "rollout", quoteId: q.id } : { kind: "quote", quoteId: q.id });
      if (!account.onAccount) cardPaidOrderIds.add(order.id);
  ...
  const cancelled = new Set(rng.sample(salesOrders.filter((o) => origins.get(o.id)?.kind === "cycle" && daysBetween(o.createdAt, today) > 60), 2).map((o) => o.id));
  const partDeliveredCandidates = new Set(
    rng.sample(salesOrders.filter((o) => origins.get(o.id)?.kind === "quote" && daysBetween(o.createdAt, today) > 12 && daysBetween(o.createdAt, today) < 60 && o.lines.length > 1), 2).map((o) => o.id),
  );

  const addDelivery = (order: SalesOrder, lines: { productId: string; qty: number }[], dispatched: Date, delivered: Date | null) => {
    ...
    deliveries.push({
      ...
      status: delivered ? "delivered" : daysBetween(dispatched, today) >= 1 ? "in-transit" : "dispatched",
      carrier,
      trackingRef: carrier === "Brewfitt delivery" ? null : `${carrier.slice(0, 3).toUpperCase()}${rng.int(10_000_000, 99_999_999)}`,
      dispatchedAt: isoDateTime(dispatched, rng.int(7, 9), rng.pick([0, 30])),
      deliveredAt: delivered ? isoDateTime(delivered, rng.int(10, 16), rng.pick([5, 25, 45])) : null,
      lines,
      noteDocumentId: null,
  ...
  for (const order of salesOrders) {
    ...
    if (origin.kind === "rollout") {
      ...
      order.lines = order.lines.map((l) =>
        l.productId === slugId("cobra-4-out-chrome-led") ? { ...l, delivered: 8, backordered: 4 } : { ...l, delivered: l.qty, backordered: 0 },
      );
      addDelivery(order, order.lines.map((l) => ({ productId: l.productId, qty: l.delivered })), dispatched, addDays(dispatched, 1));
    ...
    if (age < dispatchAfter + 1) {
      ...
      addDelivery(order, order.lines.map((l) => ({ productId: l.productId, qty: l.qty })), workingDay(addDays(created, Math.min(age, dispatchAfter))), null);
      continue;
    }
    ...
    if (partDeliveredCandidates.has(order.id)) {
      ...
      order.lines = order.lines.map((l) => (l === short ? { ...l, delivered: Math.floor(l.qty / 2), backordered: l.qty - Math.floor(l.qty / 2) } : { ...l, delivered: l.qty }));
      addDelivery(order, order.lines.filter((l) => l.delivered > 0).map((l) => ({ productId: l.productId, qty: l.delivered })), dispatched, delivered);
      order.status = "part-delivered";
      continue;
    }
    ...
    if (order.lines.length > 2 && rng.chance(0.3) && age > 20) {
      ...
      addDelivery(order, rest.map((l) => ({ productId: l.productId, qty: l.qty })), second, addDays(second, 1));
    } else {
      addDelivery(order, order.lines.map((l) => ({ productId: l.productId, qty: l.qty })), dispatched, delivered);
    }
  ...
  for (const order of rng.sample(open, Math.min(3, open.length))) {
    ...
    changeRequests.push({
      ...
      current,
      status: "pending",
      reason: rng.pick(["Cellar refurbishment runs over, so we cannot take delivery that day.", "Our manager is on leave that week; please deliver after they return.", "We need the delivery after the weekend event."]),
      createdAt: isoDateTime(addDays(today, -rng.int(0, 1)), rng.int(9, 16)),
    });
  }
  const delivered = salesOrders.filter((o) => o.status === "delivered" && daysBetween(o.createdAt, today) < 120);
  for (const [i, order] of rng.sample(delivered, 3).entries()) {
    const altAddress = addresses.find((a) => a.accountId === order.accountId && a.id !== order.deliveryAddressId);
    ...
    changeRequests.push({
      ...
      current: kind === "address" ? order.deliveryAddressId : isoDate(addDays(new Date(`${order.confirmedDate}T00:00:00Z`), -2)),
      status: i === 2 ? "rejected" : "approved",
      reason: i === 2 ? "The order had already been picked for the original address." : "Approved; delivery moved to the requested date.",
  ...
  const jobs: Job[] = [];
  const addJob = (order: SalesOrder, name: string, scheduled: Date, siteAddressId: string, engineer: string) => {
  ...
  addJob(rolloutOrder, "Premium Lager font install: wave 1", workingDay(addDays(today, -16)), rolloutStores[0]!.id, "Craig Lockwood");
  addJob(rolloutOrder, "Premium Lager font install: wave 2", workingDay(addDays(today, 0)), rolloutStores[1]!.id, "Dean Sykes");
  addJob(rolloutOrder, "Premium Lager font install: wave 3 (back-ordered fonts)", workingDay(addDays(today, 19)), rolloutStores[0]!.id, "Craig Lockwood");
  ...
  const installOrders = salesOrders.filter((o) => {
    ...
    return installSectors.has(acc.parentAccountId ? "pub" : acc.sector) && ctx.vatRate(o.accountId) > 0 && net > 80_000;
  });
  for (const order of installOrders) {
    const lastDelivery = deliveries.filter((d) => d.orderId === order.id).map((d) => d.deliveredAt ?? d.dispatchedAt).sort().pop();
    const base = lastDelivery ? new Date(lastDelivery) : new Date(`${order.confirmedDate}T00:00:00Z`);
    const scheduled = workingDay(addDays(base, rng.int(2, 7)));
    const config = configurations.find((c) => c.quoteId === order.quoteId);
    const label = config ? config.name : `${ctx.productById.get(order.lines[0]!.productId)!.name.split(" ").slice(0, 3).join(" ")} install`;
    ...
    if (big && jobs.length < 26) {
      ...
      addJob(order, `${label}: second fix and commissioning`, workingDay(addDays(scheduled, rng.int(3, 6))), order.deliveryAddressId, rng.pick(ENGINEERS));
    } else {
  ...
  const lowStockProductIds = [slugId("pipeline-purple-beer-line-cleaning-powder"), slugId("s-type-keg-coupler-c-w-jg-fittings-nrv-sankey")];
