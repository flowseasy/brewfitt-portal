import type { Delivery, Offer, Product, PurchaseOrder, Rfq, SalesOrder, StockPosition, SupplierProduct, SupplierQuote } from "@/types";
...
export function seedPurchasing(ctx: SeedContext, commerce: { salesOrders: SalesOrder[]; deliveries: Delivery[]; rolloutOrderId: string }): PurchasingSeed {
  ...
  const costFor = (supplierId: string, productId: string) => {
    ...
    const line = ctx.priceListLines.find((l) => l.priceListId === s.priceListId && l.productId === productId);
  ...
  for (const o of commerce.salesOrders) for (const l of o.lines) sold.set(l.productId, (sold.get(l.productId) ?? 0) + l.qty);

  const purchaseOrders: PurchaseOrder[] = [];
  const makePo = (supplierId: string, created: Date, lines: { productId: string; qty: number }[]): PurchaseOrder => {
    const priced = lines.map((l) => ({ ...l, received: 0, price: costFor(supplierId, l.productId) }));
    ...
    const po: PurchaseOrder = {
      ...
      threadId: "",
      createdAt: isoDateTime(previousWorkingDay(created), rng.int(9, 15), rng.pick([0, 15, 30, 45])),
    };
  ...
  for (const supplier of suppliers) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const created = i === count - 1 && count >= 4 ? addDays(today, -rng.int(2, 9)) : ctx.seasonalDate(360, 25);
      const picks = rng.sample(ranked.slice(0, Math.max(4, Math.ceil(ranked.length / 3))), Math.min(ranked.length, rng.int(2, 5)));
      makePo(
        supplier.id,
        created,
        picks.map((p) => ({ productId: p.id, qty: Math.max(ctx.roleOf(p) === "project" ? rng.int(2, 8) : rng.int(10, 40), Math.ceil((sold.get(p.id) ?? 0) / 4)) })),
      );
    }
  }
  ...
  for (const po of purchaseOrders) {
    ...
    const inbound = (lines: { productId: string; qty: number }[], dispatched: Date, delivered: Date | null) =>
      inboundDeliveries.push({
        ...
        carrier,
        trackingRef: carrier === "Supplier transport" ? null : `${carrier.slice(0, 3).toUpperCase()}${rng.int(10_000_000, 99_999_999)}`,
    ...
    if (age <= 1) {
      ...
      inbound(po.lines.map((l) => ({ productId: l.productId, qty: l.qty })), workingDay(addDays(expected, -2)), null);
    } else if (age < 40 && po.lines.length > 2 && rng.chance(0.5)) {
      po.status = "part-received";
      po.lines = po.lines.map((l, i) => (i === 0 ? { ...l, received: Math.floor(l.qty / 2) } : { ...l, received: l.qty }));
      inbound(po.lines.map((l) => ({ productId: l.productId, qty: l.received })).filter((l) => l.qty > 0), workingDay(addDays(expected, -2)), expected);
    } else {
      po.status = "received";
      po.lines = po.lines.map((l) => ({ ...l, received: l.qty }));
      inbound(po.lines.map((l) => ({ productId: l.productId, qty: l.qty })), workingDay(addDays(expected, -2)), expected);
    }
  }
  ...
  for (const po of purchaseOrders) {
    ...
    for (const l of po.lines) {
      ...
      onOrder.set(l.productId, { qty: (e?.qty ?? 0) + outstanding, expected: e && e.expected < po.expectedDate ? e.expected : po.expectedDate });
    }
  }
  ...
  const stock: StockPosition[] = products.map((p: Product) => {
    ...
    const minimumLevel = forced[p.id]?.minimumLevel ?? (role === "consumable" ? rng.int(15, 30) : role === "spare" ? rng.int(4, 12) : p.listPrice.amount > 150_000 ? 1 : 2);
    ...
    if (onHand === undefined) {
      ...
      onHand = roll < 0.015 ? 0 : roll < 0.04 ? Math.max(0, minimumLevel - rng.int(1, 3)) : minimumLevel + rng.int(role === "project" ? 0 : 5, role === "consumable" ? 120 : role === "spare" ? 50 : 8);
      onHand = Math.max(onHand, alloc);
    }
    const available = onHand - alloc;
    const status: StockPosition["status"] =
      available <= 0 ? (order ? "on-order" : "out") : onHand < minimumLevel ? (order ? "on-order" : "low") : "in-stock";
    return {
      productId: p.id,
      onHand,
      allocated: alloc,
  ...
  rfqPlan.forEach(([supplierId, status], i) => {
    const created = status === "open" ? addDays(today, -rng.int(1, 6)) : addDays(today, -rng.int(20, 150));
    ...
    if (status !== "open" && status !== "closed") {
      supplierQuotes.push({
        ...
        notes: rng.pick(["Prices held for 60 days.", "Includes delivery to Huddersfield.", "Volume price applies if called off in two drops.", null]),
        submittedAt: isoDateTime(addDays(created, rng.int(2, 6)), 14),
      });
    }
  });
  rfqs.sort((a, b) => a.createdAt.localeCompare(b.createdAt)).forEach((r) => (r.number = ctx.number("rfq")));

  // ---- Supplier product submissions ----------------------------------------
  const submissionPlan: { supplierId: string; slug: string | null; name: string; status: SupplierProduct["status"]; note: string | null; ageDays: number }[] = [
    { supplierId: "sup_vireo", slug: "classic-cobra-2-out-led", name: "Classic Cobra 2 Out LED", status: "approved", note: "Approved with spec sheet. Now live in TOTA360v5.", ageDays: 64 },
    { supplierId: "sup_vireo", slug: "cobra-4-out-chrome-led", name: "Cobra B 4 Out Chrome LED", status: "approved", note: "Spec sheet and images approved.", ageDays: 38 },
    { supplierId: "sup_vireo", slug: null, name: "Cobra B 5 Out Chrome LED", status: "under-review", note: null, ageDays: 6 },
    { supplierId: "sup_vireo", slug: null, name: "Matt Black FC4 Tap (Lager)", status: "submitted", note: null, ageDays: 2 },
    { supplierId: "sup_vireo", slug: null, name: "Brushed Brass Badge Holder", status: "rejected", note: "Images are renders only; please resubmit with photographs of the finished product and a spec sheet.", ageDays: 45 },
    { supplierId: "sup_brightbar", slug: "st-st-drip-tray-50x22x3-c-w-glass-freshener-drain", name: "St/St Drip Tray 50x22x3 C/W Glass Freshener & Drain", status: "approved", note: "Approved; spec sheet attached to the catalogue item.", ageDays: 90 },
    { supplierId: "sup_brightbar", slug: null, name: "Slimline LED Board 600mm", status: "under-review", note: null, ageDays: 9 },
    { supplierId: "sup_tapmobile", slug: "lindr-kontakt-70k-2-taps", name: "Lindr Kontakt 70K (2 Taps)", status: "approved", note: "Spec sheet approved.", ageDays: 120 },
    { supplierId: "sup_tapmobile", slug: null, name: "Lindr Kontakt 40K Green Line (2 Taps)", status: "submitted", note: null, ageDays: 4 },
    { supplierId: "sup_clearflow", slug: "pipeline-purple-beer-line-cleaning-powder", name: "Pipeline Purple Professional Beer Line Cleaning Powder (10 Sachets)", status: "approved", note: "Safety data and spec sheet approved.", ageDays: 150 },
    { supplierId: "sup_clearflow", slug: null, name: "Low-foam Line Cleaner Concentrate (5 Litres)", status: "rejected", note: "We need the safety data sheet before this can be listed.", ageDays: 30 },
    { supplierId: "sup_northgas", slug: null, name: "Secondary Mixed Gas Regulator Panel (4 Way)", status: "under-review", note: null, ageDays: 12 },
    { supplierId: "sup_polarflex", slug: null, name: "10 Line Python Loom (per metre)", status: "submitted", note: null, ageDays: 3 },
    { supplierId: "sup_emerald", slug: "coldfusion-nitro-infusion-system", name: "ColdFusion Nitro infusion system", status: "approved", note: "Approved.", ageDays: 200 },
  ];
  const supplierProducts: SupplierProduct[] = submissionPlan.map((s, i) => {
    ...
    const category = product?.category ?? rng.pick(products.filter((p) => p.supplierId === s.supplierId)).category;
    const similar = products.filter((p) => p.supplierId === s.supplierId && p.category === category);
    const cost = product ? costFor(s.supplierId, product.id) : money(rng.pick(similar).listPrice.amount * 0.55);
    const submitted = addDays(today, -s.ageDays);
    return {
      id: `sp_${i + 1}`,
      supplierId: s.supplierId,
      name: s.name,
      sku: product?.sku ?? `${ctx.account(s.supplierId).name.slice(0, 3).toUpperCase()}-${rng.int(1000, 9999)}`,
      category,
      ...
      updatedAt: isoDateTime(addDays(submitted, s.status === "submitted" ? 0 : Math.min(s.ageDays, rng.int(2, 10))), 15),
    };
  });

  // ---- Offers ---------------------------------------------------------------
  const offerPlan: [string, Offer["status"], number, number, string][] = [
    ["sup_vireo", "approved", -10, 40, "Spring rollout pricing on Cobra fonts ordered in multiples of six."],
    ["sup_vireo", "submitted", 2, 60, "Introductory price on FC4 chrome taps for orders of 50 or more."],
    ...
    ["sup_brightbar", "under-review", 0, 45, "Recessed drip trays with glass freshener at volume pricing."],
  ...
  const offers: Offer[] = offerPlan.map(([supplierId, status, fromDays, toDays, description], i) => {
    const range = products.filter((p) => p.supplierId === supplierId);
  ...
  return { purchaseOrders, inboundDeliveries, rfqs, supplierQuotes, supplierProducts, offers, stock };
}
