      ...
      <DialogContent className="flex max-h-[94dvh] w-[calc(100vw-1rem)] flex-col gap-0 overflow-hidden p-0 sm:max-w-4xl">
        <DialogHeader className="flex-row items-center justify-between gap-3 border-b px-5 py-3">
          <div>
            ...
            <DialogDescription className="text-xs">Preview of the PDF produced by TOTA360v5</DialogDescription>
          </div>
        ...
        <div className="overflow-y-auto bg-muted/60 p-3 sm:p-6">
          <article data-print-root className="mx-auto max-w-[210mm] bg-white p-6 text-[13px] leading-relaxed text-neutral-900 shadow-sm ring-1 ring-black/5 sm:p-10">
            {children}
          </article>
        </div>
      </DialogContent>
...
export function DocumentHeader({ kind, number, date, meta }: { kind: string; number?: string; date: string; meta?: { label: string; value: string }[] }) {
  return (
...
export function DocumentFooter({ note }: { note?: string }) {
  return (
    <footer className="mt-10 border-t border-neutral-200 pt-4 text-[11px] text-neutral-500">
      ...
      <p>Brewfitt Limited. Prices exclude VAT unless shown. Phase 1 preview generated from demonstration data.</p>
    </footer>
  );
}
