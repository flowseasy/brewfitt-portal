...
import { FileImageIcon, FilePdfIcon, FileTextIcon, FileVideoIcon, FileXlsIcon } from "@phosphor-icons/react/dist/ssr";
import type { Document } from "@/types";
import { daysFromToday, formatDate, formatShortDate } from "@/lib/format";
import { hrefFor } from "@/lib/links";
import { StatusPill } from "./status-pill";

const FILE_ICON = { pdf: FilePdfIcon, jpg: FileImageIcon, png: FileImageIcon, docx: FileTextIcon, xlsx: FileXlsIcon, mp4: FileVideoIcon };
...
export function ExpiryPill({ expiresAt }: { expiresAt: string | null }) {
  ...
  if (days <= 30) return <StatusPill tone="warning">Expires in {days} {days === 1 ? "day" : "days"}</StatusPill>;
...
export function DocumentCard({ doc, showApproval = true }: { doc: Document; showApproval?: boolean }) {
  const Icon = FILE_ICON[doc.fileType];
  return (
    <Link href={hrefFor("document", doc.id)} className="flex items-center gap-3 rounded-xl border bg-card p-3 transition hover:border-primary/40 hover:bg-accent/40">
