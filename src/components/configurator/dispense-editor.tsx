...
export function DispenseEditor({
  points,
  rules,
  dispatch,
}: {
  points: DispensePoint[];
  rules: ConfiguratorRules;
  dispatch: (a: BuilderAction) => void;
}) {
  const set = (next: DispensePoint[]) => dispatch({ type: "points", points: next });
  const update = (id: string, patch: Partial<DispensePoint>) =>
    set(points.map((p) => (p.id === id ? { ...p, ...patch } : p)));
  const nextId = () =>
    `pt-${Math.max(0, ...points.map((p) => Number(p.id.replace("pt-", "")) || 0)) + 1}`;
      ...
      <div className="mb-3 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Each point is one font or tower on the bar. Add a product for every tap.
          </p>
        </div>
        <Button
          type="button"
          ...
          onClick={() =>
            set([
              ...points,
              { id: nextId(), name: `Dispense point ${points.length + 1}`, taps: ["lager"] },
            ])
          }
        >
          ...
          return (
            <li key={p.id} className="rounded-xl border bg-background p-3">
              <div className="flex items-center gap-2">
                <span
                  className="flex size-7 shrink-0 items-center justify-center rounded-full bg-brand-subtle text-xs font-semibold text-brand-subtle-foreground"
                  aria-hidden
                >
                  {index + 1}
                </span>
                ...
                <Button
                  ...
                  variant="ghost"
                  size="icon-sm"
                  onClick={() => set(points.filter((x) => x.id !== p.id))}
                  disabled={points.length <= rules.limits.minPoints}
              ...
              <ul className="mt-2 flex flex-wrap gap-2" aria-label={`Products on ${p.name}`}>
                {p.taps.map((tap, i) => (
                  <li
                    key={`${p.id}-${i}`}
                    className="flex items-center rounded-full border bg-card pr-1 pl-3"
                  >
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        update(p.id, {
                          taps: p.taps.map((t, j) => (j === i ? (e.target.value as Beverage) : t)),
                        })
                      }
