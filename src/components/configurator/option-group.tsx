...
import { checkCompatibility, type BuilderAction, type BuilderState } from "@/features/configurator/use-configurator";
...
export function ConfiguratorOptionGroup({
  ...
  const selected = new Set([...state.selections.dispense.optionIds, ...state.selections.font.optionIds, ...state.selections.cooling.optionIds, ...state.selections.gas.optionIds, ...state.selections.ancillaries.optionIds]);
  ...
  const optionCost = (optionId: string) => {
    ...
    return buildBillOfMaterials(probe, { ...rules, options: rules.options.filter((o) => o.id === optionId) }, (id) => prices.get(id) ?? null).total;
  };
    ...
    <fieldset aria-labelledby={legendId} className="rounded-2xl border bg-card p-4 sm:p-5">
      <div className="mb-3">
        <legend id={legendId} className="flex items-center gap-2 font-medium">
          {group.label}
          {group.required && !auto ? <span className="text-xs font-normal text-muted-foreground">Required</span> : <span className="text-xs font-normal text-muted-foreground">{auto ? "Added for you" : "Optional"}</span>}
        </legend>
        {group.help ? <p className="mt-0.5 text-sm text-muted-foreground">{group.help}</p> : null}
      </div>

      {compatible.length === 0 ? (
        <p className="rounded-xl bg-muted/60 p-3 text-sm text-muted-foreground">Not needed for this configuration.</p>
      ) : (
            ...
            return (
              ...
              >
                <input
                  ...
                  checked={checked}
                  disabled={auto}
                  onChange={(e) => dispatch({ type: "select", groupId: group.id, optionId: o.id, single, on: single ? true : e.target.checked })}
                  onClick={(e) => {
                    // Radios in optional groups can be cleared by choosing them again.
                    if (single && checked && !group.required) {
                      e.preventDefault();
                      dispatch({ type: "select", groupId: group.id, optionId: o.id, single, on: false });
                    }
                  }}
                  className="sr-only"
                />
                ...
                <span className="min-w-0 flex-1">
                  <span className="flex items-start justify-between gap-2">
                    ...
                    {checked ? <CheckCircleIcon weight="fill" className="size-5 shrink-0 text-primary" aria-hidden /> : <span aria-hidden className="size-5 shrink-0 rounded-full border" />}
                  </span>
                  <span className="mt-0.5 block text-xs text-muted-foreground">{o.description}</span>
                  {cost.amount > 0 ? <span className="mt-1 block text-xs font-medium tabular-nums">+{formatMoney(cost)}</span> : null}
                </span>
              </label>
            );
          })}
      ...
      {unavailable.length && !auto ? (
        <div className="mt-3">
          <button type="button" onClick={() => setShowUnavailable((v) => !v)} aria-expanded={showUnavailable} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground">
            <CaretDownIcon className={cn("size-3.5 transition-transform", showUnavailable && "rotate-180")} aria-hidden />
            {unavailable.length} {unavailable.length === 1 ? "option does" : "options do"} not suit this configuration
          </button>
          {showUnavailable ? (
            <ul className="mt-2 space-y-1 text-xs text-muted-foreground">
              {unavailable.map(({ option, result }) => (
                <li key={option.id}>
                  <span className="font-medium text-foreground">{option.label}</span>: {result.compatible ? "" : result.reason}
                </li>
              ))}
            </ul>
          ) : null}
