...
export function AssistantButton() {
  ...
  return (
    <Button
      ...
      onClick={() => openPanel()}
      aria-label="Assistant and insights"
    >
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetContent className="flex w-full flex-col gap-0 p-0 sm:max-w-md">
        <SheetHeader className="border-b pb-0">
          ...
          <SheetDescription>
            Simulated insights and answers from your own records. Phase 1 uses fixed rules on
            demonstration data.
          </SheetDescription>
          <div role="tablist" aria-label="Assistant" className="mt-2 -mb-px flex gap-4">
            {tabs.map((t) => (
              <button
                key={t.value}
                role="tab"
                ...
                className={cn(
                  "border-b-2 px-1 pb-2.5 text-sm font-medium transition focus-visible:ring-3 focus-visible:ring-ring/40 focus-visible:outline-none",
                  tab === t.value
        ...
        {tab === "ask" ? (
          ...
          <div
            role="tabpanel"
            id="assistant-panel-insights"
            aria-labelledby="assistant-tab-insights"
...
export function InsightList() {
  ...
  if (insights.isError)
    return <ErrorState error={insights.error} onRetry={() => insights.refetch()} />;
  ...
  if (all.length === 0) {
    return (
      <EmptyState
        ...
        description={
          supplier
            ? "Insights appear when Brewfitt's stock of your items runs low or a purchase is likely."
            : "Insights appear when a regular item is due, stock is running low, a quote is about to expire or an invoice is due."
        }
      />
    );
  }
        ...
        <div className="mb-4 flex flex-wrap gap-1.5" role="group" aria-label="Filter by category">
          {(["all", ...categories] as const).map((c) => (
            ...
            >
              {c === "all"
                ? "All"
                : `${CATEGORY_LABEL[c]} (${all.filter((i) => i.category === c).length})`}
            </button>
          ))}
        </div>
      ) : null}
