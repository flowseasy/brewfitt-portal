...
const CaseForm = z
  .object({
    ...
    description: z
      .string()
      .trim()
      .min(20, "Describe the problem in a sentence or two so an engineer can triage it"),
    productId: z.string(),
    orderId: z.string(),
    jobId: z.string(),
    photos: z.array(z.string()),
  })
  .refine((c) => c.productId !== NONE || c.orderId !== NONE || c.jobId !== NONE, {
    message: "Link the case to a product, order or install job",
    path: ["productId"],
  });
...
export function RaiseCaseSheet({
  ...
  defaults?: { orderId?: string; jobId?: string; productId?: string };
}) {
  ...
  const create = useMutation({
    mutationFn: (v: CaseValues) =>
      api.cases.create({
        ...v,
        productId: v.productId === NONE ? null : v.productId,
        orderId: v.orderId === NONE ? null : v.orderId,
        jobId: v.jobId === NONE ? null : v.jobId,
      }),
    onSuccess: (c) => {
      ...
      onOpenChange(false);
      form.reset();
      toast.success(`Case ${c.number} raised`, {
        description: "Brewfitt's technical team will reply in the case conversation.",
      });
      router.push(hrefFor("case", c.id));
    },
    onError: (error) =>
      toast.error("The case could not be raised", { description: errorMessage(error) }),
  });
        ...
        >
          <FieldGroup>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(Object.keys(CASE_KIND_LABEL) as Case["kind"][]).map((k) => (
                      <label
                        key={k}
                        className={cn(
                          "flex cursor-pointer items-center justify-center rounded-xl border p-2.5 text-sm focus-within:ring-3 focus-within:ring-ring/40",
            ...
            <TextField
              ...
              name="subject"
              label="Subject"
              placeholder="Lager font fobbing on every first pour"
            />
            <TextareaField
              control={form.control}
              name="description"
              ...
              rows={4}
              placeholder="When it started, which taps or lines, what you have already checked"
            />
            ...
            <SelectField
              ...
              options={[
                ...
                ...(orders.data ?? [])
                  .slice(0, 40)
                  .map((o) => ({ value: o.id, label: `${o.number} · ${formatDate(o.createdAt)}` })),
              ]}
            />
            <SelectField
              control={form.control}
            ...
            <Field>
              ...
              <FieldDescription>
                Photos of the fault help an engineer bring the right parts. In this preview only
                file names are sent.
              </FieldDescription>
            </Field>
          </FieldGroup>
