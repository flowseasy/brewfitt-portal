...
- Internal Brewfitt staff views: team workload, pipeline reports, employee map, leadership dashboards (exception: the Composite Configurator, decision 14)
...
- Scripts: `npm run dev`, `typecheck`, `lint`, `build` (static export to `out/`), `format`, `gen:types`, `check:data` (seed consistency for today and +45/+120/+250 days), `check:api` (every endpoint and mutation per persona, reload replay, reset), `test` (unit tests, `*.test.ts` via node:test).
...
- Ageing bands are by due date so they agree with Overdue (changed after client-demo review): `current` = not yet due, `30` = 1–30 days overdue, `60` = 31–60, `90+` = over 60 days overdue. `ageingBand` and `overdue` status are computed at read time from the clock. The AI invoice-ageing rule also uses days past `dueAt`.
...
- Contract additions defined by the mock (blueprint silent): `Me.credit` and `Me.vatRate`; `Statement.unallocatedCredit`; `supplierProducts.performance()`; `messages.threads()` returns `ThreadSummary` (thread plus `lastMessage`). `account.stats()` (customer statistics) and the extended `supplierProducts.performance()` (supplier statistics) feed the dashboard panels; `PurchaseOrder.acknowledgedAt` records acknowledgement time. "Spend, last 6 months vs the 6 before" stands in for year-on-year because the demo holds 12 months of history. Record PDFs open via `?pdf=1` on quote, order and invoice views.
...
12. **Customer Support and Supplier Support (user, after M18):** "Cases" is labelled Customer Support. Suppliers get Supplier Support on the same `/cases` routes and `Case` record: kinds `purchase-order`, `payment`, `delivery`, `product-listing`, `general`, linked by `purchaseOrderId` / `invoiceId` (or one of their products), answered by Brewfitt's buyer. Customer kinds and links (order, job) stay customer-only; `check:data` and the mock enforce the split. Supplier purchase order and invoice pages link to "Raise an issue". DB_VERSION 4.
13. **Dispense Designer (user, 18 September 2026):** customers see the configurator as **Dispense Designer**: sidebar label (directly below Shop), landing page "Your Dispense Designs", "New design" button, breadcrumbs, onboarding, quote links, message labels, error messages and quote-request text say "design", never "configuration" or "configurator". The name "Configurator" is reserved for a separate internal tool. Internal code names stay as they are (route `/configurator`, `configurator` API, `Configuration` types, `src/components/configurator/`, `src/lib/configurator/`).
14. **Composite Configurator (user, 18 September 2026; an exception to "no internal staff views", and BLUEPRINT.md has no section for it, so the user's two specs are the source, the revised one winning):** a Brewfitt-staff-only tool for costing made-to-order composites. A **Brewfitt staff** persona (Sam Ridley, Brewfitt Limited, `kind: "staff"`; synthetic `BREWFITT_ACCOUNT` (`kind: "internal"`) and `STAFF_CONTACT` in `src/lib/mock/seed/composite.ts`, outside the customer data) sees only **Configurator** (`/internal/configurator`, `/internal/configurator/build?id=`) and read-only **Quotes** (`/internal/quotes`, `/internal/quotes/view?id=`), under a "Brewfitt internal" header band (`--internal` tokens) with no assistant, notifications, search or customer CTAs, and skips onboarding. `requireStaff` guards the `internal` API group (`/api/internal/...`: composite settings, customers, cost-item type-ahead, composite builds list/get/create/update/duplicate/add-to-quote, quotes); `requireCustomer` refuses staff.
    - Model (`src/schemas/composite.ts`): `CompositeBuild` with header (name, customer, brand/project, date, creator initials, customer-facing description), per-build FX (`fxRates`, units per £1; mock system rates USD 1.30 / EUR 1.17 with the 0.05 buffer note) and labour rate (default £12/hr), and `CompositeBand`s (`base`, `50`, `100`, `200`; a new band copies base). Each band has its own lines (CODE, DESCRIPTION, FX COST, £ COST, QTY, TOTAL; catalogue, component or Misc with selling/buying names and descriptions and `supplierQuoteRef`), shipping and duty (amount per unit or % of goods), labour hours, target margin, sell price and carriage.
    - Pricing (`src/lib/composite/pricing.ts`, shared by UI, mock and tests): £ cost = FX cost ÷ rate, unrounded (fractions of a penny as the sheet shows); cost total = goods + shipping + duty + labour (labour is costed as a line, so it is marked up; the team's sheet adds it after margin instead, and it is zero in the example. Kept as built on the user's instruction; **Brewfitt to confirm** which treatment they want); price by margin = cost ÷ (1 − margin); margin = GP ÷ sell; markup = GP ÷ cost; carriage (CRG) is added after margin, not marked up. Typing a margin sets the price; typing a price sets the margin; both are shown.
    - Seed: 300 invented components (mock `MC-XXX-0000` codes, not Brewfitt's) plus the catalogue at cost (supplier agreed price, else 55% of list) for the type-ahead; six draft builds, including the Krusovice tap handle reproduced from the team's costing sheet (USD 1.25, 50+/100+/200+ at £45/£34/£29, margins 29.04/30.56/27.85%). `npm test` (node:test via tsx) asserts those figures; `check:data` and `check:api` also check them.
    - Add to quote: one line (`QuoteLine.kind: "composite"`) with the composite name, description (`QuoteLine.detail`) and the band price only, `productId: null`; when the band has carriage, a separate **Delivery** line (`kind: "delivery"`, qty 1, carriage × quantity) follows it. The composite line carries the BOM in `QuoteLine.internal`, which `customerQuote()` strips from every customer response, so it never reaches customer quote views, the PDF preview or messages. A new quote is sent ready to accept (PDF, conversation from the account manager, notification), or the line is added to an open quote. Accepting a quote with composite lines orders only stocked product lines (`salesOrder` may be null) and Brewfitt replies that it is setting the composite up as a product. "Convert to composite" is disabled: "Available in TOTA360v5 in Phase 2". The seeded Krusovice build keeps the sheet's creator initials "JP"; the staff persona is the invented Sam Ridley (SR). The team's Excel sheet is not in the repo. DB_VERSION 7.
