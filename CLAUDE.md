...
14. **Composite Configurator (user, 18 September 2026; an exception to "no internal staff views", and BLUEPRINT.md has no section for it, so the user's two specs are the source, the revised one winning):** a Brewfitt-staff-only tool for costing made-to-order composites. A **Brewfitt staff** persona (James Pollard, Brewfitt Limited, `kind: "staff"`; synthetic `BREWFITT_ACCOUNT` (`kind: "internal"`) and `STAFF_CONTACT` in `src/lib/mock/seed/composite.ts`, outside the customer data) sees only **Configurator** (`/internal/configurator`, `/internal/configurator/build?id=`) and read-only **Quotes** (`/internal/quotes`, `/internal/quotes/view?id=`), under a "Brewfitt internal" header band (`--internal` tokens) with no assistant, notifications, search or customer CTAs, and skips onboarding. `requireStaff` guards the `internal` API group (`/api/internal/...`: composite settings, customers, cost-item type-ahead, composite builds list/get/create/update/duplicate/add-to-quote, quotes); `requireCustomer` refuses staff.
    ...
    - Add to quote: one line with the composite name, description (`QuoteLine.detail`) and band price (+ carriage), `productId: null`, and the BOM in `QuoteLine.internal`, which `customerQuote()` strips from every customer response, so it never reaches customer quote views, the PDF preview or messages. A new quote is sent ready to accept (PDF, conversation from the account manager, notification), or the line is added to an open quote. Accepting a quote with composite lines orders only stocked lines (`salesOrder` may be null) and Brewfitt replies that it is setting the composite up as a product. "Convert to composite" is disabled: "Available in TOTA360v5 in Phase 2". The team's Excel sheet is not in the repo. DB_VERSION 7.
